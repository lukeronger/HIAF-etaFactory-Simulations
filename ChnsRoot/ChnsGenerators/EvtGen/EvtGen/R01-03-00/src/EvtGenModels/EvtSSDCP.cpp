...
void EvtSSDCP::init(){
  ...
  if ( ( getParentId() != EvtPDL::getId( "B0" ) )
    ...
    report(EVT_ERROR, "EvtGen" ) << "EvtSSDCP only decays B0 and B0s" 
                               << std::endl ;
    ::abort() ;
  }
  ...
  if ( (!(d1type == EvtSpinType::SCALAR || d2type == EvtSpinType::SCALAR))||
    ...
    report(EVT_ERROR,"EvtGen") << "EvtSSDCP generator expected "
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen") << "SSD_CP will generate CP/CPT violation:"
			  << endl << endl
