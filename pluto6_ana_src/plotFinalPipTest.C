{
	...
	TFile ff("../../hiaf_MCdata/pp_to_ppeta_PipPimPi0_1.8GeV.root");
	...
	cout<<data->Draw("Particles.TLorentzVector.P():Particles.TLorentzVector.Theta()*TMath::RadToDeg()>>h(200,0,180,200,0,2)","Particles.pid==7","colz")<<endl;
...
//	li->Draw();
	...
	//gPad->SetLogz();
	h->SetTitle("");
	h->GetXaxis()->SetTitle("#theta^{#pi^{0}} (#circ)");
	h->GetXaxis()->SetTitleSize(0.06);
	h->GetXaxis()->CenterTitle();
	h->GetXaxis()->SetTitleOffset(1.05);
	h->GetXaxis()->SetLabelSize(0.06);
	h->GetYaxis()->SetTitle("P^{#pi^{0}} (GeV/c)");
