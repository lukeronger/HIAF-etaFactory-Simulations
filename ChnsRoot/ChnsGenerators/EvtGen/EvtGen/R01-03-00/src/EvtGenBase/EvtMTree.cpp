...
vector<EvtMNode *> EvtMTree::makeparticles( const string& strid ) 
{
    ...
    if( labels.size() == 0 ) {
        report(EVT_ERROR,"EvtGen")<<"Error unknown particle label "<<strid<<endl;
        ::abort();
    }
        ...
        const vector<EvtComplex>& amps, const vector<EvtMNode *>& children )
{
    ...
    if( ls=="BREITWIGNER" ) {
        ...
        report(EVT_ERROR,"EvtGen")<<"Lineshape "<<lineshape
                              <<" not recognized."<<endl;
        ::abort();
    }
    ...
    if( type=="HELAMP" ) {
        ...
        report(EVT_ERROR,"EvtGen")<<"Model "<<type<<" not recognized."<<endl;
        ::abort();
    }
...
{ 
    ...
    report(EVT_ERROR,"EvtGen")<<"Parse error at: "<<error<<endl;
    ::abort();
}
...
vector<EvtMNode *> EvtMTree::parsenode( const string& args, bool rootnode )
{
    ...
    report(EVT_ERROR,"EvtGen")<<children.size()<<endl;
...
EvtSpinAmp EvtMTree::amplitude( EvtParticle * p ) const
{
    ...
    if( _root.size() == 0 ) {
        report(EVT_ERROR, "EvtGen")<<"No decay tree present."<<endl;
        ::abort();
    }
