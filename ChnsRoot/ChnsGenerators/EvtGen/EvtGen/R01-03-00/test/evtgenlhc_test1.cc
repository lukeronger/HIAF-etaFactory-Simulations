...
int main(int argc, char* argv[]){
  ...
  if (argc==1){
    ...
    report(EVT_INFO,"EvtGen") << "p:"<<p<<std::endl;
    report(EVT_INFO,"EvtGen") << "k:"<<k<<std::endl;
    report(EVT_INFO,"EvtGen") << "T=dual(directProd(p,k)):"<<T<<std::endl;
    report(EVT_INFO,"EvtGen") << "T03:"<<T.get(0,3)<<std::endl;
    return 1;
  }
...
void runFile(int nevent,char* fname, EvtGen &myGenerator) {
  ...
  report(EVT_INFO,"EvtGen") << "SUCCESS\n";
}
...
void runPi0Dalitz(int nevent, EvtGen &myGenerator) {
  ...
  do{
      ...
      q2->Fill( (ep+em).mass2() );
      //      report(EVT_INFO,"EvtGen") << ep << em << gamma <<std::endl;
      root_part->deleteTree();  
...
void runDDK(int nevent, EvtGen &myGenerator) {
  ...
  report(EVT_INFO,"EvtGen") << nDDK << " " << (count-1) << " " << nDDK/float(2*(count-1)) << std::endl;
  report(EVT_INFO,"EvtGen") << "SUCCESS\n";
}
...
void runAlias() {
  ...
  report(EVT_INFO,"EvtGen") << "Id    pi+:" << idpip << std::endl;
  report(EVT_INFO,"EvtGen") << "Id    pi-:" << idpim << std::endl;
  report(EVT_INFO,"EvtGen") << "Id    pi0:" << idpi0 << std::endl;
  report(EVT_INFO,"EvtGen") << "Id my_pi+:" << myidpip << std::endl;
  report(EVT_INFO,"EvtGen") << "Id my_pi-:" << myidpim << std::endl;
  report(EVT_INFO,"EvtGen") << "Id my_pi0:" << myidpi0 << std::endl;
  
  report(EVT_INFO,"EvtGen") << "Chg conj    pi+:" << 
    EvtPDL::chargeConj(idpip) << std::endl;
  report(EVT_INFO,"EvtGen") << "Chg conj    pi-:" << 
    EvtPDL::chargeConj(idpim) << std::endl;
  report(EVT_INFO,"EvtGen") << "Chg conj    pi0:" << 
    EvtPDL::chargeConj(idpi0) << std::endl;
  report(EVT_INFO,"EvtGen") << "Chg conj my_pi+:" << 
    EvtPDL::chargeConj(myidpip) << std::endl;
  report(EVT_INFO,"EvtGen") << "Chg conj my_pi-:" << 
    EvtPDL::chargeConj(myidpim) << std::endl;
  report(EVT_INFO,"EvtGen") << "Chg conj my_pi0:" << 
...
void runFinalStates(int nevent, EvtGen &myGenerator) {
  ...
  do{
    ...
    if (count==1000*(count/1000)) {
      //if (count==1*(count/1)) {
      report(EVT_INFO,"EvtGen") << "Event:"<< count << std::endl;
      //report(EVT_INFO,"EvtGen") << HepRandom::getTheSeed()<<std::endl;
    }
	...
	for(k=0; k<temp.size(); k++) {
	  ...
	  for(l=0; l<temp.size(); l++) {
	    ...
	    if ( temp[k]==fs[l] ) { 
	      alreadyUsed[l]=true; 
	      foundThisOne=true; 
	      //	      report(EVT_INFO,"EvtGen") << "found daughter " << k << " " << l << std::endl;
	    }
	  }
	  if ( !foundThisOne ) foundIt=false;
	}
	if ( foundIt ) {//report(EVT_INFO,"EvtGen") << "found a cand \n"; (histo1[j])->Fill(0.5);
	  //report(EVT_INFO,"EvtGen") << "found one " << j << std::endl; 
	dListNum[j]++;}
      }
    }
  ...
  int j;
  for (j=0; j<(tline-1) ; j++) report(EVT_INFO,"EvtGen") << dListName[j].c_str() << " " << j << " " << dListNum[j] << " " << count << " "<< (dListNum[j]/(1.0*count)) << std::endl;
...
void runTrackMult(int nevent, EvtGen &myGenerator) {
  ...
  do{
    ...
    if (count==1000*(count/1000)) {
      report(EVT_INFO,"EvtGen") << count << std::endl;
      //report(EVT_INFO,"EvtGen") << HepRandom::getTheSeed()<<std::endl;
    }
    EvtVector4R p_init(EvtPDL::getMass(UPS4),0.0,0.0,0.0);
  ...
  report(EVT_INFO,"EvtGen") << "Your average multiplicity="<<aveMulti<<std::endl;
  report(EVT_INFO,"EvtGen") << "Your average multiplicity for no B->semileptonic events="<<aveMultiNoSL<<std::endl;
  report(EVT_INFO,"EvtGen") << "Your average multiplicity for 1 B->semileptonic events="<<aveMulti1SL<<std::endl;
  report(EVT_INFO,"EvtGen") << "Your average multiplicity for 2 B->semileptonic events="<<aveMulti2SL<<std::endl;
  file->Write(); file->Close();
  report(EVT_INFO,"EvtGen") << "SUCCESS\n";
...
void runGeneric(int neventOrig, EvtGen &myGenerator, 
  ...
  int nevent=abs(neventOrig);
	...
	report(EVT_ERROR,"EvtGen") << "Will terminate."<<std::endl;
	exit(-1);
      }
	...
	delete massName;
      ...
      do{
	//report(EVT_INFO,"EvtGen") << "new event\n";
	if (count==1000*(count/1000)) {
	  report(EVT_INFO,"EvtGen") << count << std::endl;
	  //report(EVT_INFO,"EvtGen") << HepRandom::getTheSeed()<<std::endl;
	}
	...
	//report(EVT_INFO,"EvtGen") <<stdhep<<std::endl;
	//report(EVT_INFO,"EvtGen") <<secondary<<std::endl;
	std::string token;
	int itok;
...
void runKstarll(int nevent, EvtGen &myGenerator) {
  ...
  do{
    ...
    report(EVT_INFO,"EvtGen") << "count:"<<count<<" "<<(l1+l2).mass2()<<std::endl;
...
void runKstarstargamma(int nevent, EvtGen &myGenerator) {
  ...
  do{
    ...
    //report(EVT_INFO,"EvtGen") << "ctheta:"<<EvtDecayAngle(pinit,p4_kaon+p4_pion,p4_kaon)<<std::endl;
...
void runPartWave2(int nevent, EvtGen &myGenerator) {
  ...
  do{
    ...
    //report(EVT_INFO,"EvtGen") << "costhetan:"<<costhetan<<std::endl;
    ...
    //report(EVT_INFO,"EvtGen") << "costhetarho:"<<costhrho<<std::endl;
...
void runTwoBody(int nevent, EvtGen &myGenerator, std::string decFile,
  ...
  do{
    ...
    do {
      ...
      if (!(nDaug==0||nDaug==2)) {
	report(EVT_INFO,"EvtGen") << "nDaug="<<nDaug<<" but can only handle 0 or 2!"<<std::endl;
	abort();
      }
...
void runBtoXsgamma(int nevent, EvtGen &myGenerator) {
 ...
 // Plot kinematics for b->s,gamma
  ...
  do{
      ...
      if (Bmulti==1) report(EVT_INFO,"EvtGen") << "B1" << " bId1a=" << bId1a << " bId1b=" << bId1b << " ndaug=" << B1->getNDaug() << " Bid=" << EvtPDL::getStdHep(B1->getId()) << std::endl;
     ...
     if (Bmulti==1) report(EVT_INFO,"EvtGen") << "B2" << " bId2a=" << bId2a << " bId2b=" << bId2b << " ndaug=" << B2->getNDaug() << " Bid=" << EvtPDL::getStdHep(B2->getId()) << std::endl;
      ...
      report(EVT_INFO,"EvtGen") << "bId1a "<<bId1a<<" bId1b "<<bId1b<<" bId2a "<<bId2a<<" bId2b "<<bId2b<<" for event "<<count<<std::endl;
      ...
      if (pengcount == 0) {
        Bpeng=B1;
        report(EVT_INFO,"EvtGen") << "No penguin decay for event "<<count<<std::endl;
        bnum=0;
      } else if (pengcount == 2) {
        Bpeng=B1;
        report(EVT_INFO,"EvtGen") << "Two penguin decays in event "<<count<<std::endl;
        bnum=0;        
      }
...
// debug stuff:      report(EVT_INFO,"EvtGen") << "bnum=" << bnum << " pengcount=" << pengcount << " bId=" << bId << " Bmulti=" << Bmulti << " XsId=" << XId << " gId=" << gId << std::endl;
  ...
  report(EVT_INFO,"EvtGen")<<"End EvtGen. Ran on "<<nevent<<" events."<<std::endl;
  report(EVT_INFO,"EvtGen") << "SUCCESS\n";
...
// debug stuff:      report(EVT_INFO,"EvtGen") << "bnum=" << bnum << " pengcount=" << pengcount << " bId=" << bId << " Bmulti=" << Bmulti << " KsId=" << XId << " gId=" << gId << std::endl;
...
void runCheckRotBoost(){
  ...
  report(EVT_INFO,"EvtGen") << "Comparing after doing a random boost"<<std::endl;
  report(EVT_INFO,"EvtGen") << "Scalar "<<s<<" "<<sBoost<<s-sBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "PseudoScalar "<<p<<" "<<pBoost<<p-pBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "AxialVector "<<aDirBoost<<" "<<aBoost<<aDirBoost-aBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "Vector "<<vDirBoost<<" "<<vBoost<<vDirBoost-vBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "V-A "<<vaDirBoost<<" "<<vaBoost<<vaDirBoost-vaBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "Tensor "<<tDirBoost<<" "<<tBoost<<tDirBoost-tBoost<<std::endl;
  report(EVT_INFO,"EvtGen") << "Done comparing after doing a random boost"<<std::endl;
  ...
  report(EVT_INFO,"EvtGen") << "Comparing after doing a random rotation"<<std::endl;
  report(EVT_INFO,"EvtGen") << "Scalar "<<s<<" "<<sRot<<s-sRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "PseudoScalar "<<p<<" "<<pRot<<p-pRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "AxialVector "<<aDirRot<<" "<<aRot<<aDirRot-aRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "Vector "<<vDirRot<<" "<<vRot<<vDirRot-vRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "V-A "<<vaDirRot<<" "<<vaRot<<vaDirRot-vaRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "Tensor "<<tDirRot<<" "<<tRot<<tDirRot-tRot<<std::endl;
  report(EVT_INFO,"EvtGen") << "Done comparing after doing a random rotation"<<std::endl;
  report(EVT_INFO,"EvtGen") << "SUCCESS\n";
...
int countInclusive(std::string name, EvtParticle *root_part,TH1F* mom, TH1F* mass){
  ...
  do{
    ...
    if (type==searchFor) { 
      ...
      //}	  
      //report(EVT_INFO,"EvtGen") << "LANGE " << p->getP4Lab().d3mag() << " " << p->getP4Lab().get(3)/p->getP4Lab().d3mag() << std::endl;
    }
...
int countInclusiveSubTree(std::string name, EvtParticle *root_part, 
  ...
  //report(EVT_INFO,"EvtGen") << "done"<<std::endl;
  return temp;
} 
...
void runPiPiPiPi(int nevent, EvtGen &myGenerator) {
	...
	(p4pi1+p4pi2).mass() <<  " " <<
      ...
      report(EVT_INFO,"EvtGen") << "count:"<<count<<std::endl;
...
void runGenericCont(int nevent, EvtGen &myGenerator) {
    ...
    do{
      ...
      if (count==1000*(count/1000)) {
       report(EVT_DEBUG,"EvtGen") << count << std::endl;
...
void runD1(int nevent, EvtGen &myGenerator) {
    ...
    do{
      ...
      report(EVT_DEBUG,"EvtGen") << "count:"<<count<<std::endl;
