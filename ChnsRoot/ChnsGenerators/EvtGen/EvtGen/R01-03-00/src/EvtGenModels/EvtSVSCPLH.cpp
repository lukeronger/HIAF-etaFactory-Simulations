...
void EvtSVSCPLH::init(){
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen")<<":EvtSVSCPLH:dm="<<_dm<<endl;
    report(EVT_INFO,"EvtGen")<<":EvtSVSCPLH:dGamma="<<_dgamma<<endl;
    report(EVT_INFO,"EvtGen")<<":EvtSVSCPLH:q/p="<<_qop<<endl;
    report(EVT_INFO,"EvtGen")<<":EvtSVSCPLH:Af="<<_Af<<endl;
    report(EVT_INFO,"EvtGen")<<":EvtSVSCPLH:Abarf="<<_Abarf<<endl;
  }
...
void EvtSVSCPLH::decay( EvtParticle *p){
  ...
  if (other_b==B0B){
    ...
    report(EVT_ERROR,"EvtGen") << "other_b was not B0 or B0B!"<<endl;
    ::abort();
  }
