...
ChnsFsmEmcFS::ChnsFsmEmcFS(ArgList &par)
{
  initParameters();
...
ChnsFsmResponse*
ChnsFsmEmcFS::respond(ChnsFsmTrack *t)
{
  ...
  if (wasDetected)
  {
    ...
    t->setStopVtx(hitpos);  //fix me later. xy position need to be recalculated for charged particles.
  }
  else
  {
    result->setdE(0.);
...
bool
ChnsFsmEmcFS::detected(ChnsFsmTrack *t) const
{
  if (t->hitMapValid()) {
...
double
ChnsFsmEmcFS::dE(ChnsFsmTrack *t) const
{
  double E = t->p4().E();

  //return (sqrt(_aPar*_aPar+_bPar*_bPar/E ) * E);  //Jerzy's version
  return (sqrt(_aPar*_aPar+_bPar*_bPar/sqrt(E) ) * E);   //12.03 2018  Yutie
}

double
ChnsFsmEmcFS::dphi(ChnsFsmTrack *t) const
{
...
double
ChnsFsmEmcFS::dtheta(ChnsFsmTrack *t) const
{
...
double
ChnsFsmEmcFS::gauss(double x, double x0, double s)
{
...
double
ChnsFsmEmcFS::compdepE(double E, double pdgcode)
{
...
bool
ChnsFsmEmcFS::setParameter(std::string &name, double value)
{
  // *****************
