...
void EvtVubNLO::init(){
  ...
  if (getNArg()<npar) {
    ...
    report(EVT_ERROR,"EvtGen") << "EvtVubNLO generator expected "
  ...
  if (maxw == 0) {
    report(EVT_ERROR,"EvtGen") << "EvtVubNLO generator expected at least one " 
...
void EvtVubNLO::decay( EvtParticle *p ){
  ...
  while (tryit) {
    ...
    if (pp<pl && El>ml&& sh > _masses[0]*_masses[0]&& _mB*_mB + sh - 2*_mB*Eh > ml*ml) {
      ...
      if(pdf>_dGMax){
	report(EVT_ERROR,"EvtGen") << "EvtVubNLO pdf above maximum: " <<pdf
...
double EvtVubNLO::SFNorm(  const std::vector<double> &/*coeffs*/){
  ...
  if(_idSF==1){ // exponential SF
    ...
    report(EVT_ERROR,"EvtGen")  << "unknown SF "<<_idSF<<endl;
    return -1;
  }
}
...
EvtVubNLO::shapeFunction ( double omega, const std::vector<double> &sCoeffs){
 ...
 report(EVT_ERROR,"EvtGen") << "EvtVubNLO : unknown shape function # "
			   <<sCoeffs[6]<<endl;
  }
  return -1.;
}
