...
!
    ...
    real :: Q2max,numax,W2max,Eprime,cost,pL,pT,plept
    integer :: iHist
...
! The next statements return a sampled neutrino energy for given flux
! distributions.
! Specific experiments 1 - 25 have prestored flux distributions
! that are being sampled.
! Experiment 99 accepts any new, outside flux distribution
! The flux file for exp 99 must be a .txt file with two columns:
! 1st col: energy, 2nd col: flux, the energies must be equidistant
          ...
          if (nuExp.gt.0) then
             ...
             case (15)
                flux_enu=DUNEenergyNU(Flavor_ID,Process_ID)
             case (16)
                flux_enu=DUNEenergyBARNU(Flavor_ID,Process_ID)
             case (17)
             ...
             case (99)
                flux_enu=userFlux(FileNameFlux)
             case default
                write(*,*) 'Experiment does not exist'
                call TRACEBACK()
             end select
!
! The following statement takes any possible flux-cuts into account
!
...
! the following line adds the sampled energy into a flux histogram
! The flux distribution is reflected in the density of sampled energies
...
! output is in files named neutrino.*****.dat

! The next loop runs over the IDs of all possible final states
! max_finalstate_ID set earlier in this file
!
! first all inclusive cross sections are calculated, then a MC decision is made
! which process actually takes place and is further propagated


          particle_ID_loop: do k=1, max_finalstate_ID    
            ! at present max_.. = 37, see line 280
             if (.not.includeK(k)) cycle

             eNev(k) = eNev1 
            ! the eN event picked out above without consideration
            ! of final state is now associated with the final state k			 

! Set outgoing particles for DIS and 2pi separately, for all other
! channels obtained from the cross section routines
...
! The field 'sigma' contains the cross sections of the various subprocesses,
! as calculated above, the 'total weight' gives the total cross section,
! summed over all subprocesses. 'k' is then the statistically chosen final state
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!     Now build histograms of inclusive cross sections
!     that depend only on lepton kinematics
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          case (2)
             call neutrinoProdInfo_Store(firstEvent, k, fak1,&
                  ...
                  eNev(k)%lepton_out%momentum,&
                  eNev(k)%boson%momentum,&
                  eNev(k)%nucleon%momentum,&
                  eNev(k)%nucleon%charge)
          end select
  ...
  !        Write(*,*) 'Wfree', eNev(k)%W_free
          ...
          Eprime = eNev(k)%lepton_out%momentum(0)
          cost = eNeV_Get_CostLepton(eNev(k))
          plept = abs3(eNev(k)%lepton_out%momentum)
          pL = plept * cost
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! The following files contain inclusive X-sections. They do not contain effects
! of any kinematical cuts on the lepton or the hadrons.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
