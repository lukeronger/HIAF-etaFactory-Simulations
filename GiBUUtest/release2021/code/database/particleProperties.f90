...
Module ParticleProperties
  ...
  ! PURPOSE
  ! * 0 = use resonances with more than 1 star rating for cross sections
  ! * 1 = use all resonances for cross sections
  ! * 2 = use all resonances besides the 1* star I=1/2 resonances
  ...
  !****************************************************************************
  Interface validCharge
     Module Procedure validCharge_P, validCharge_ID
  end Interface validCharge
...
!     end subroutine TestOldVersion
  ...
  !****************************************************************************
  ! cf. interface 'validCharge' :
  !****************************************************************************
  logical function validCharge_P(part)
  ...
  logical function validCharge_ID(ID, Q)
    ...
    if (isMeson(id)) then
       Iz2 = 2*Q - (hadron(id)%strangeness+hadron(id)%charm)
    else if (isBaryon(id)) then
       Iz2 = 2*Q - (1+hadron(id)%strangeness+hadron(id)%charm)
    else
       if (Id==0 .or. Id==photon) then
         validCharge_ID=.true.
         return
       end if
       write(*,*) 'Error in validCharge: Particle is unknown:', id, Q
       call Traceback('stop')
    end if

    max_Iz2 = hadron(id)%isoSpinTimes2

    validCharge_ID = (abs(Iz2) <= max_Iz2)
