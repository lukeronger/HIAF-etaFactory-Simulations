...
void EvtAmp::setNState(int parent_states,int *daug_states){
  ...
  if (_nontrivial>5) {
    report(EVT_ERROR,"EvtGen") << "Too many nontrivial states in EvtAmp!"<<endl;
  }
...
EvtSpinDensity EvtAmp::getSpinDensity(){
  ...
  if (_pstates==1) {
	...
	//        if (_nontrivial>3){
	//report(EVT_ERROR,"EvtGen") << "Can't handle so many states in EvtAmp!"<<endl;
	//}
...
EvtAmp EvtAmp::contract(int k,const EvtSpinDensity& rho){
  ...
  if (_nontrivial==0) {
    report(EVT_ERROR,"EvtGen")<<"Should not be here EvtAmp!"<<endl;
  }
...
EvtSpinDensity EvtAmp::contract(int k,const EvtAmp& amp2){
  ...
  for(i=0;i<_nstate[k];i++){
    ...
    for(j=0;j<_nstate[k];j++){
      if (_nontrivial==0) {
	report(EVT_ERROR,"EvtGen")<<"Should not be here1 EvtAmp!"<<endl;
        rho.set(0,0,EvtComplex(1.0,0.0)); 
        return rho;
      }
...
EvtAmp EvtAmp::contract(int , const EvtAmp& ,const EvtAmp& ){
  ...
  EvtAmp tmp;
  report(EVT_DEBUG,"EvtGen") << "EvtAmp::contract not written yet" << endl;
  return tmp;
...
void EvtAmp::dump(){
  ...
  report(EVT_DEBUG,"EvtGen") << "Number of daugthers:"<<_ndaug<<endl;
  report(EVT_DEBUG,"EvtGen") << "Number of states of the parent:"<<_pstates<<endl;
  report(EVT_DEBUG,"EvtGen") << "Number of states on daughters:";
  for (i=0;i<_ndaug;i++){
    report(EVT_DEBUG,"EvtGen") <<dstates[i]<<" ";
  }
  report(EVT_DEBUG,"EvtGen") << endl;
  report(EVT_DEBUG,"EvtGen") << "Nontrivial index of  daughters:";
  for (i=0;i<_ndaug;i++){
    report(EVT_DEBUG,"EvtGen") <<_dnontrivial[i]<<" ";
  }
  report(EVT_DEBUG,"EvtGen") <<endl;
  report(EVT_DEBUG,"EvtGen") <<"number of nontrivial states:"<<_nontrivial<<endl;
  report(EVT_DEBUG,"EvtGen") << "Nontrivial particles number of states:";
  for (i=0;i<_nontrivial;i++){
    report(EVT_DEBUG,"EvtGen") <<_nstate[i]<<" ";
  }
  report(EVT_DEBUG,"EvtGen") <<endl;
  report(EVT_DEBUG,"EvtGen") <<"Amplitudes:"<<endl;
  if (_nontrivial==0){
    list[0] = 0;
    report(EVT_DEBUG,"EvtGen") << getAmp(list) << endl;
  }
  ...
  for (i=0;i<allloop[_nontrivial-1];i++) {
    report(EVT_DEBUG,"EvtGen") << getAmp(list) << " ";
