...
EvtPhotosEngine::EvtPhotosEngine(std::string photonType, bool useEvtGenRandom) {
  ...
  report(EVT_INFO,"EvtGen")<<"Setting up PHOTOS."<<endl;

  if (useEvtGenRandom == true) {
      
    report(EVT_INFO,"EvtGen")<<"Using EvtGen random number engine also for Photos++"<<endl;
...
void EvtPhotosEngine::initialise() {
  ...
  if (_initialised == false) {
    ...
    if (_gammaId == EvtId(-1,-1)) {
      report(EVT_INFO,"EvtGen")<<"Error in EvtPhotosEngine. Do not recognise the photon type "
