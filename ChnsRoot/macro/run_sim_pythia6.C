void run_sim_pythia6(Int_t nEvents = 10, TString mcEngine = "TGeant4", Bool_t isMT = false)
{
    ...
    mvd->SetGeometryFileName("genfitGeom_AllSi_v2.gdml");
    run->AddModule(mvd);
    ...
    if(UseBoxGenerator){
	    ...
	    boxGen->SetPRange(5.0, 10.0);                             // GeV/c
	    boxGen->SetPhiRange(0., 360.);                            // Azimuth angle range [degree]
	    boxGen->SetThetaRange(60., 120.);                          // Polar angle in lab system range [degree]
    ...
    if(UsePythia6){
	    TString  EvtInput =gSystem->Getenv("VMCWORKDIR");
	    EvtInput+="/macro/pythia_example.txt";
            cout<<EvtInput<<endl;
            Pythia6Generator* pythia6 = new Pythia6Generator(EvtInput.Data());
            primGen->AddGenerator(pythia6);
    }
