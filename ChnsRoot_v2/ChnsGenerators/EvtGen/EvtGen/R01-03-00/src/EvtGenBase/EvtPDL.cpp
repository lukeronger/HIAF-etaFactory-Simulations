...
void EvtPDL::readPDT(const std::string fname){
  ...
  if (!indec) {
    report(EVT_ERROR,"EvtGen") << "Could not open:"<<fname.c_str()<<"EvtPDL"<<endl;
    return;
  }
  ...
  do{
	...
	tmp.setName(pname);
	    ...
	    report(EVT_ERROR,"EvtGen")<<"The particle name:"<<pname<<" is already defined."<<endl;
	    report(EVT_ERROR,"EvtGen") << "Will terminate execution.";
	    ::abort();
	}
...
void EvtPDL::aliasChgConj(EvtId a,EvtId abar){
  ...
  if (EvtPDL::chargeConj(EvtId(a.getId(),a.getId()))!=
    ...
    report(EVT_ERROR,"EvtGen")<<"Can't charge conjugate the two aliases:"
...
EvtId EvtPDL::chargeConj(EvtId id){
  ...
  if (id.getAlias()!=id.getId()) {
    ...
    report(EVT_ERROR,"EvtGen")<<"Trying to charge conjugate alias particle:"
...
void EvtPDL::alias(EvtId num,const std::string& newname){
  ...
  if ( _firstAlias < partlist().size() ) {
	...
	report(EVT_WARNING,"EvtGen")<<"Redefining alias:"<<newname.c_str()<<" will be ignored!"<<endl;
	return;
      }
    }
  }
  ...
  if (_particleNameLookup.find(std::string(newname))!=
      _particleNameLookup.end()){
	    report(EVT_ERROR,"EvtGen")<<"The particle name:"<<newname<<" is already defined."<<endl;
	    report(EVT_ERROR,"EvtGen") << "Will terminate execution.";
	    ::abort();
  }
