...
!!$  end function hadronicPotEnergy
  ...
  subroutine init
    ...
    allocate(densityField(-gridPoints(1):gridPoints(1),&
    ...
    if ( getRMF_flag() ) then
       ...
       allocate(rhoField(-gridPoints(1):gridPoints(1),&
            &            -gridPoints(2):gridPoints(2),&
            &            -gridPoints(3):gridPoints(3), 0:3))
       allocate(scalarDensity(-gridPoints(1):gridPoints(1),&
            &                 -gridPoints(2):gridPoints(2),&
            &                 -gridPoints(3):gridPoints(3)))
  ...
  subroutine cleanUp
    ...
    if ( getRMF_flag() ) then
       deallocate(sigmaField,omegaField,rhoField,scalarDensity, &
            d_scalarDensity_dsigma,omegaField_old, &
  ...
  subroutine acceptGrid(GridSpacing_x,GridSpacing_y,GridSpacing_z)
    ...
    PosT(1:3) = targetNuc%position(1:3) &
         + targetNuc%radius + targetNuc%surface + 5.
    PosP(1:3) = projectileNuc%position(1:3) &
  ...
  real function getBaryonDensity(pos)
    ...
    if (all(abs(i(:))<=gridPoints(:))) then
       ...
       getBaryonDensity = (densityField(i(1),i(2),i(3))%baryon(0) &
            + factor*totalDensity(i(1),i(2),i(3))) / (1.+factor)
  ...
  type(dichte) function densityAt(r)
    ...
    case (1) ! dynamic density according to test-particle distribution
       ...
       if (GetGridIndex(r,indizes,0)) then   ! inside grid
          ...
          if (continousBoundaries) then
             ...
             if (GetGridIndex(rNew,indizes,0)) then  ! inside grid
                if (linearInterpolation) then
      ...
      do i=0,7
         ...
         if ((abs(grid(i,1)).le.gridPoints(1)) &
            ...
            interpolate=interpolate &
                 + densityField(grid(i,1),grid(i,2),grid(i,3))*factor
         end if
      end do
  ...
  subroutine updateDensity(Parts)
    ...
    !    use coulomb, only: getCoulombFlag ! this is a circular dependency !!!
...
!    logical :: doCoul
    ...
    densityField = dichte0
    totalDensity = 0.

!    doCoul = getCoulombFlag()
  ...
  contains
    ...
    subroutine addTodensityfield(ind)
      ...
      if (isBaryon(pPart%id)) then
         ...
         if (getRMF_flag()) then
            ...
            ! Modification factor for coupling constants (always positive !!!):
      ...
      densityField(I1,I2,I3)%charge = densityField(I1,I2,I3)%charge &
           & + vec * pPart%charge
    ...
    subroutine initDensityWeights
      ...
      allocate(smearingWeights(1:(2*smallerGridPoints+1)**3,&
           &                   1:(2*numberLargepoints+1)**3))
...
!!$            end if
      ...
      do indexSmall1=-smallerGridPoints,smallerGridPoints
         do indexSmall2=-smallerGridPoints,smallerGridPoints
            do indexSmall3=-smallerGridPoints,smallerGridPoints
               small=1+(smallerGridPoints+indexSmall3)&
  ...
  subroutine updateRMF(Parts,flagini_)
    ...
    if(present(flagini_)) then
       if(flagini_) flagini=.true.
    end if
    ...
    if (.not.flagini .and. densitySwitch.ne.1) then
       ...
       do j=1,Size(Parts,dim=1)
          do i=1,Size(Parts,dim=2)
             ...
             if(abs(Parts(j,i)%OffShellParameter).gt.1.e-06) cycle
  ...
  ! are given.
  ! INPUTS
  ! * type(particle),intent(inOut) :: Parts  -- Particle whose energy should
  !   be calculated.
  ! NOTES
  ! Should be used in RMF-mode. Please, notice, that not the full
  ...
  subroutine energyDeterminationRMF(Parts)
    ...
    if ( (isBaryon(Parts%Id) .or. Parts%Id.eq.Kaon .or. Parts%Id.eq.kaonBar) &
       ...
       case (rho,omegaMeson,phi)
          ...
          density = densityAt(Parts%position)
          densityLRF=abs4(density%baryon,flagOk)
          if(flagOk) then
             spot = vecMes_massShift(Parts%id,densityLRF)
          else
             spot=0.
          end if
       case default
       ...
       Parts%momentum(0) = sqrt( (Parts%mass+spot)**2 + pstar2 )
  ...
  ! OUTPUT
  ! * real, dimension(0:3) :: momentum --- canonical 4-momentum of the particle
  ! NOTES
  ...
  ! INPUTS
  ! * type(particle) :: teilchen --- particle whose "true" 4-momentum should
  !   be calculated.
  ! OUTPUT
  ...
  subroutine true4Momentum_RMF(teilchen,momentum,inside_grid_flag)
    ...
    use RMF, only: ModificationFactor, g_2, g_3, g_omega, g_rho, g_sigma, &
         lorentz_flag
    ...
    InsideGrid : if (  abs(I1) < gridPoints(1) .and. &
       ...
       !           endif
       !        else
       !           isofact=0. !isospin sector presently only for p and n
       !        endif
  ...
  end subroutine set_densityInput
       ...
       do i1 = -gridPoints(2),gridPoints(2)
          do i2 = -gridPoints(3),gridPoints(3)
             ...
             write(iFile, '(3f12.4,1P,16e13.5)') &
                  r,densityField(indexOrigin(1),i1,i2)
          end do
          write(iFile,*)
       end do
    ...
    case (2)
       do i1 = -gridPoints(1),gridPoints(1)
          do i2 = -gridPoints(3),gridPoints(3)
             ...
             write(iFile, '(3f12.4,1P,16e13.5)') &
                  r,densityField(i1,indexOrigin(2),i2)
          end do
          write(iFile,*)
       end do
    ...
    case (3)
       do i1 = -gridPoints(1),gridPoints(1)
          do i2 = -gridPoints(2),gridPoints(2)
             ...
             write(iFile, '(3f12.4,1P,16e13.5)') &
                  r,densityField(i1,i2,indexOrigin(2))
          end do
          write(iFile,*)
       end do
