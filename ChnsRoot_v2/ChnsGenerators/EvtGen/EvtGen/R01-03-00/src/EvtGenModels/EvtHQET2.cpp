...
void EvtHQET2::init(){
  ...
  if ( d1type==EvtSpinType::SCALAR) {
    ...
    report(EVT_ERROR,"EvtGen") << "HQET2 model for scalar meson daughters needs 2 arguments. Sorry."<<endl;
    ::abort();
  }  
  }
    ...
    report(EVT_ERROR,"EvtGen") << "HQET2 model for vector meson daughtersneeds 4 arguments. Sorry."<<endl;
    ::abort();
    }
  }
  else{
    report(EVT_ERROR,"EvtGen") << "HQET2 model handles only scalar and vector meson daughters. Sorry."<<endl;
    ::abort();
  }
