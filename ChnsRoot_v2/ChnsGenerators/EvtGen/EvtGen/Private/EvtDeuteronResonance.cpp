...
void EvtDeuteronResonance::init() {
  ...
  if (getNArg()==4) {
    ...
    if(_pmax<0.2 || _pmax>1.5) {
      report(EVTGEN_ERROR,"EvtGen") << "Maximum spectator nucleon momentum should be within [0.2, 1.5] GeV/c!" << endl;
    ...
    report(EVTGEN_ERROR,"EvtGen") << "EvtDeuteronResonance generator expects "
  ...
  report(EVTGEN_INFO,"EvtGen") << "The deuteron spectator generator has been configured:"
    <<endl;
