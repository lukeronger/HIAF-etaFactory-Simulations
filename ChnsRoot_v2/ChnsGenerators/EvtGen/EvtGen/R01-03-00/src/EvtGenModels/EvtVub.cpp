...
void EvtVub::init(){
  ...
  if (getNArg()<6) {
    ...
    report(EVT_ERROR,"EvtGen") << "EvtVub generator expected "
  ...
  if (maxw == 0) {
    report(EVT_ERROR,"EvtGen") << "EvtVub generator expected at least one " 
  ...
  while(rew){
	...
	if ( Eh > 0 && Eh < mB ) {
	  ...
	  if ( sh > _masses[0]*_masses[0]
	    ...
	    if ( y > 1 ) report(EVT_WARNING,"EvtGen")<<"EvtVub decay probability > 1 found: " << y << endl;
	    if ( y >= xran ) tryit = 0;
	  }
	}
      }
