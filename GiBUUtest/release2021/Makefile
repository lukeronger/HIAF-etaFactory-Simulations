...
# starting with gfortran 6, MATMUL is inlined when optimizing.
# This yields different results in e.g hadronTensor_ResProd.f90
# Therefore this inlining is switched off as workaround.
# versions 7.4, 8.3 and 9 are okay again and do not need this workaround.
  HASNOMATMULINLINE:=$(shell LANG=c $(FORT) -finline-matmul-limit=0 2>&1 | grep -c 'unrecognized command line option')

  FORTVERS_MAJOR:=$(strip $(shell echo $(FORTVERS) | tr a-zA-Z\- ' ' | cut -f1 -d.))
  FORTVERS_MINOR:=$(strip $(shell echo $(FORTVERS) | tr a-zA-Z\- ' ' | cut -f2 -d.))

  VERS_OKAY:=$(shell [ $(FORTVERS_MAJOR) -ge 9 -o \( $(FORTVERS_MAJOR) -eq 8 -a $(FORTVERS_MINOR) -ge 3 \) ] && echo true)

  ifeq ($(HASNOMATMULINLINE),0)
    ifneq ($(VERS_OKAY),true)
      FLAGSFORALL += -finline-matmul-limit=0
    endif
  endif

  FLAGSFORALL += -g -fbacktrace

# avoid warnings about obsolete/deleted features according the latest fortran
# standard:
...
doku:
	...
	@echo "  Generated from $(VERSION) with ROBODoc `robodoc --version` on `date` "  >> ../Documentation/index.html
...
.PHONY : Makefiles MAKEFILEveryclean MAKEFILEclean
Makefiles: #print
...
MAKEFILEdelete:
	@$(ECHO) $(StartHeader)Deleting Makefiles ...$(EndHeader)
	@for X in $(SUBDIR); do \
...
MAKEFILEveryclean: MAKEFILEdelete MAKEFILEclean
...
.PHONY : buildRootTuple

buildRootTuple: printCompiler
	@cd $(LIBDIRSRC)/RootTuple && $(MAKE) LIBROOTTUPLE
