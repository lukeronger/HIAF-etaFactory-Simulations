...
void EvtGoityRoberts::decay( EvtParticle *p){
  ...
  if (meson==DST0||meson==DSTP||meson==DSTM||meson==DSTB) {
    ...
    if (meson==D0||meson==DP||meson==DM||meson==D0B) {
      ...
      report(EVT_ERROR,"EvtGen") << "Wrong daugther in EvtGoityRoberts!\n";
    }
  }
  return ;
}
...
// Isgur&Wise fct
  ...
  //report(EVT_INFO,"EvtGen") <<"rho's:"<<rho1<<rho2<<endl;
  ...
  if (nlep==EM||nlep==MUM){ 
   ...
   report(EVT_DEBUG,"EvtGen") << "42387dfs878w wrong lepton number\n";
    }
 }
...
// Sum
  ...
  if ( nlep==EM|| nlep==MUM ) {
    ...
    if ( nlep==EP|| nlep==MUP ) {
     ...
     report(EVT_ERROR,"EvtGen") << "42387dfs878w wrong lepton number\n";
    }
  }
