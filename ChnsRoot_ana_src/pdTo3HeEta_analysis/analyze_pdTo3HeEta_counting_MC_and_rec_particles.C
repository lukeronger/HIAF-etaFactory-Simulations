...
void analyze_pdTo3HeEta_counting_MC_and_rec_particles()
{

	int total_EEGDecay_events = 1000000; 

	/// Input the simulation data from ChnsRoot
	gSystem->Load("libfsim");
	TChain* t = new TChain("chnssim");
	t->Add("/data/rwang/simulation_works/HIAF-etaFactory-Simulations-localData2/ChnsRoot-data/run_fsim_pdTo3HeEta_EEG_DecayChannel_1.8GeV_2023.root");

	/// The output file:
	TString output_file = "../../../HIAF-etaFactory-Simulations-localData/ChnsRoot-data/hiaf-pdTo3HeEta-MC-and-rec-results.root";
	...
	Int_t nn(0);
	num = total_EEGDecay_events;  /// 10000;
