...
void analyze_etaDecayDarkPhoton()
{

	int total_EEGDecay_events = 10000; 

	/// Input the simulation data from ChnsRoot
	gSystem->Load("libfsim");
	TChain* t = new TChain("chnssim");
	t->Add("/data/rwang/simulation_works/HIAF-etaFactory-Simulations-localData2/ChnsRoot-data/run_fsim_EvtGen_HIAFeta_EEG_mDarkPhoton0.05_1.8GeV_2023.root");

	/// The output file:
	TString output_file = "../../HIAF-etaFactory-Simulations-localData/ChnsRoot-data/hiaf-etaDecay-EEG-mDarkPhoton005-results.root";
	...
	TH1D * hmEE = new TH1D("hmEE","m_{ee}",900,0.01,0.58);
	TH1D * hmEE_2 = new TH1D("hmEE_2","",900,0.01,0.58);
