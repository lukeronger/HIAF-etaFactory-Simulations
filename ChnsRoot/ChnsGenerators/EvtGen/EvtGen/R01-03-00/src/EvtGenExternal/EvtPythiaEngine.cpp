...
EvtPythiaEngine::EvtPythiaEngine(std::string xmlDir, bool convertPhysCodes,
  ...
  report(EVT_INFO,"EvtGen")<<"Creating generic Pythia generator"<<endl;
  _genericPythiaGen = new Pythia8::Pythia(xmlDir);
  _genericPartData = _genericPythiaGen->particleData;

  report(EVT_INFO,"EvtGen")<<"Creating alias Pythia generator"<<endl;
...
bool EvtPythiaEngine::doDecay(EvtParticle* theParticle) {
  ...
  if (theParticle == 0) {
    report(EVT_INFO,"EvtGen")<<"Error in EvtPythiaEngine::doDecay. The mother particle is null. Not doing any Pythia decay."<<endl;
    return false;
  }
...
void EvtPythiaEngine::createDaughterEvtParticles(EvtParticle* theParent) {
  ...
  if (theParent == 0) {
    report(EVT_INFO,"EvtGen")<<"Error in EvtPythiaEngine::createDaughterEvtParticles. The parent is null"<<endl;
    return;
  }
...
void EvtPythiaEngine::updateParticleLists() {
  ...
  //report(EVT_INFO,"EvtGen")<<"Writing out changed generic Pythia decay list"<<endl;
  //_genericPythiaGen->particleData.listChanged();

  //report(EVT_INFO,"EvtGen")<<"Writing out changed alias Pythia decay list"<<endl;
...
void EvtPythiaEngine::updatePythiaDecayTable(EvtId& particleId, int aliasInt, int PDGCode) {
  ...
  for (iMode = 0; iMode < nModes; iMode++) {
	...
	report(EVT_INFO,"EvtGen")<<"Warning in EvtPythiaEngine. Trying to redefine Pythia table for "
      ...
      report(EVT_INFO,"EvtGen")<<"Error in EvtPythiaEngine. decayModel is null for particle "
...
int EvtPythiaEngine::getModeInt(EvtDecayBase* decayModel) {
  ...
  if (_convertPhysCodes) {
    ...
    if (tmpModeInt == 0) {
      ...
      report(EVT_INFO,"EvtGen")<<"Pythia mode integer "<<tmpModeInt
...
void EvtPythiaEngine::updatePhysicsParameters() {
  ...
  for( ; it!=commands.end(); it++) {
    ...
    if(command["VERSION"] == "PYTHIA6") {
      report(EVT_INFO,"EvtGen")<<"Converting Pythia 6 command: "<<command["MODULE"]<<"("<<command["PARAM"]<<")="<<command["VALUE"]<<"..."<<endl;
      ...
      report(EVT_ERROR, "EvtGen") << "Pythia command received by EvtPythiaEngine has bad version:"<<endl;
      report(EVT_ERROR, "EvtGen") << "Received "<<command["VERSION"]<<" but expected PYTHIA6 or PYTHIA8."<<endl;
      report(EVT_ERROR, "EvtGen") << "The error is likely to be in EvtDecayTable.cpp"<<endl;
      report(EVT_ERROR, "EvtGen") << "EvtGen will now abort."<<endl;
      ::abort();
    }
    ...
    if(generator == "GENERIC" || generator == "Generic" || generator == "generic" ||
      ...
      for( ; it2!=commandStrings.end(); it2++) {
        report(EVT_INFO,"EvtGen")<<"Configuring generic Pythia generator: " << (*it2) << endl;
    ...
    if(generator == "ALIAS" || generator == "Alias" || generator == "alias" ||
      ...
      std::vector<std::string>::iterator it2 = commandStrings.begin();
      for( ; it2!=commandStrings.end(); it2++) {
        report(EVT_INFO,"EvtGen")<<"Configuring alias Pythia generator: " << (*it2) << endl;
