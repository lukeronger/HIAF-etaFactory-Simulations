...
void EvtBcToNPi::init(){
	...
	if( EvtPDL::getSpinType(getDaug(0)) == EvtSpinType::VECTOR) {
		...
		report(EVT_ERROR,"EvtGen") << "Have not yet implemented this final state in BCPSINPI model" << endl;
		report(EVT_ERROR,"EvtGen") << "Ndaug="<<getNDaug() << endl;
		for ( int id=0; id<(getNDaug()-1); id++ ) 
			report(EVT_ERROR,"EvtGen") << "Daug " << id << " "<<EvtPDL::name(getDaug(id)).c_str() << endl;
		return;
...
// check pi-mesons and calculate hadronic current
	...
	if ( !foundHadCurr ) {
		report(EVT_ERROR,"EvtGen") << "Have not yet implemented this final state in BCNPI model" << endl;
		report(EVT_ERROR,"EvtGen") << "Ndaug="<<getNDaug() << endl;
		int id;
...
void EvtBcToNPi::printAuthorInfo() {
  ...
  report(EVT_INFO,"EvtGen")<<"Defining EvtBcToNPi model: Bc -> V + npi and Bc -> P + npi decays\n"
