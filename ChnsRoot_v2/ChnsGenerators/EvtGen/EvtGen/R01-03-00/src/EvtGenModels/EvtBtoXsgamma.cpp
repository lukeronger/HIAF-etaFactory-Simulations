...
void EvtBtoXsgamma::init(){
 ...
 //No more arguments for Ali-Greub model
  ...
  if (getNArg() == 0) {
    ...
    report(EVT_ERROR,"EvtGen") << "EvtBtoXsgamma generator expected "
...
void EvtBtoXsgamma::decay( EvtParticle *p ){
  ...
  if ( _model == 0 ) {
    ...
    if (getArg(0) == 1) _model = new EvtBtoXsgammaAliGreub();
      ...
      report(EVT_ERROR,"EvtGen") << "No valid EvtBtoXsgamma generator model selected "
  ...
  //  report(EVT_DEBUG,"EvtGen") << "In EvtBtoXsgamma: X_s daughters should not be here!"<<endl;
  //  return;
  //}
