...
void EvtDalitzTable::readXMLDecayFile(const std::string dec_name, bool verbose){
  ...
  if (verbose) {
    report(EVT_INFO,"EvtGen")<<"EvtDalitzTable: Reading in xml parameter file "<<dec_name<<endl;
  }
    ...
    //TAGS FOUND UNDER DALITZDECAY
      ...
      if(parser.getTagTitle() == "resonance") {
        ...
        case 2:
          ...
          break;
        default:
          report(EVT_ERROR,"EvtGen") << "Unsupported spin near line "<<parser.getLineNumber()<<" of XML file."<<endl;
          ::abort();
        }
        ...
        if(resDaugStr != "") {
          ...
          if(nResDaug != 2) {
            report(EVT_ERROR,"EvtGen") << "Resonance must have exactly 2 daughters near line "<<parser.getLineNumber()<<" of XML file."<<endl;
            ::abort();
          }
          int nRes = getDaughterPairs(resDaughter, daughter, angAndResPairs);
          if(nRes==0) {
            report(EVT_ERROR,"EvtGen") << "Resonance daughters must match decay daughters near line "<<parser.getLineNumber()<<" of XML file."<<endl;
            ::abort();
          }
        ...
        if(angAndResPairs.empty()) {
          ...
          default:
            ...
            report(EVT_ERROR,"EvtGen") << "Daughter pair must be 1, 2 or 3 near line "<<parser.getLineNumber()<<" of XML file."<<endl;
            ::abort();
          }
        }
        ...
        if(probMax < 0) {
          report(EVT_INFO,"EvtGen") << "probMax is not defined for " << decayParent << " -> " << daugStr << endl;
          report(EVT_INFO,"EvtGen") << "Will now estimate probMax. This may take a while. Once probMax is calculated, update the XML file to skip this step in future." << endl;
  ...
  if(!endReached) {
    report(EVT_ERROR,"EvtGen") << "Either the decay file ended prematurely or the file is badly formed.\n"
...
void EvtDalitzTable::copyDecay(EvtId parent, EvtId* daughters,
  ...
  report(EVT_ERROR,"EvtGen") << "Did not find dalitz decays for particle:"
         <<copy<<"\n";
}
...
EvtDalitzReso EvtDalitzTable::getResonance(std::string shape, EvtDalitzPlot dp, EvtCyclic3::Pair angPair, EvtCyclic3::Pair resPair,
  ...
  if( shape=="RBW" || shape=="RBW_CLEO") {
    ...
    if( shape!="NBW") report(EVT_WARNING,"EvtGen")<<"EvtDalitzTable: shape "<<shape<<" is unknown. Defaulting to NBW."<<endl;
...
double EvtDalitzTable::calcProbMax(EvtDalitzPlot dp, EvtDalitzDecayInfo* model) {
  ...
  report(EVT_INFO,"EvtGen") << "Largest probability found was " << maxProb << endl;
  report(EVT_INFO,"EvtGen") << "Setting probMax to " << factor*maxProb << endl;
