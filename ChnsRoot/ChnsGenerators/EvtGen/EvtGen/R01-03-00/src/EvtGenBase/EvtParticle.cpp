...
void EvtParticle::setVectorSpinDensity(){
  ...
  if (getSpinStates()!=3) {
    report(EVT_ERROR,"EvtGen")<<"Error in EvtParticle::setVectorSpinDensity"<<endl;
    report(EVT_ERROR,"EvtGen")<<"spin_states:"<<getSpinStates()<<endl;
    report(EVT_ERROR,"EvtGen")<<"particle:"<<EvtPDL::name(_id).c_str()<<endl;
    ::abort();
  }
...
void EvtParticle::decay(){
  ...
  decayer = EvtDecayTable::getInstance()->getDecayFunc(p);
  //  if ( decayer ) {
  //    report(EVT_INFO,"EvtGen") << "calling decay for " << EvtPDL::name(p->getId()) << " " << p->mass() << " " << p->getP4() << " " << p->getNDaug() << " " << p << endl;
  //    report(EVT_INFO,"EvtGen") << "NDaug= " << decayer->getNDaug() << endl;
  //    int ti;
  //    for ( ti=0; ti<decayer->getNDaug(); ti++) 
  //      report(EVT_INFO,"EvtGen") << "Daug " << ti << " " << EvtPDL::name(decayer->getDaug(ti)) << endl;
  //  }
  //if (p->_ndaug>0) {
  //      report(EVT_INFO,"EvtGen") <<"Is decaying particle with daughters!!!!!"<<endl;
  //     ::abort();
    //return;
  ...
  if (massTreeOK == false) {
    report(EVT_INFO,"EvtGen")<<"Could not decay "<<EvtPDL::name(p->getId())
			 <<" with mass "<<p->mass()
...
bool EvtParticle::generateMassTree() {
  ...
  while (massProb<ranNum) {
    ...
    if ( counter > 10000 ) {
      if ( counter == 10001 ) {
	report(EVT_INFO,"EvtGen") << "Too many iterations to determine the mass tree. Parent mass= "<< p->mass() << " " << massProb <<endl;
	p->printTree();
	report(EVT_INFO,"EvtGen") << "will take next combo with non-zero likelihood\n"; 
      }
	...
	if ( massProb>0. ) {
	  massProb=2.0;
	  report(EVT_INFO,"EvtGen") << "Taking the minimum mass of all particles in the chain\n";
	}
	else {
	  report(EVT_INFO,"EvtGen") << "Sorry, no luck finding a valid set of masses.  This may be a pathological combo\n";
	  isOK = false;
	  break;
	}
      }
...
EvtVector4C EvtParticle::epsParent(int i) const {
  EvtVector4C temp;
  printParticle();
  report(EVT_ERROR,"EvtGen") << "and you have asked for the:"<<i
...
EvtDiracSpinor EvtParticle::spParentNeutrino() const {
  EvtDiracSpinor tempD;
  printParticle();
  report(EVT_ERROR,"EvtGen") << "and you have asked for the "
			 <<"dirac spinor."
			 <<" I.e. you thought it was a"
...
void EvtParticle::printTreeRec(unsigned int level) const {
  ...
  if (_ndaug!=0) {
    ...
    report(EVT_INFO,"") << EvtPDL::name(_id).c_str();  
    report(EVT_INFO,"") << " -> ";
    for(i=0;i<_ndaug;i++){
      report(EVT_INFO,"") << EvtPDL::name(_daug[i]->getId()).c_str()<<" ";
    }
    for(i=0;i<_ndaug;i++){
      report(EVT_INFO,"") << _daug[i]->mass()<< " " << _daug[i]->getP4() << " " <<_daug[i]->getSpinStates() << "; ";
    }
...
void EvtParticle::printTree() const {
  ...
  report(EVT_INFO,"EvtGen") << "This is the current decay chain"<<endl;
  report(EVT_INFO,"") << "This top particle is "<<
    EvtPDL::name(_id).c_str()<<" " << this->mass() << " " << this->getP4() << endl;  
  
  this->printTreeRec(0);
  report(EVT_INFO,"EvtGen") << "End of decay chain."<<endl;
...
void EvtParticle::printParticle() const {
  ...
  case EvtSpinType::SCALAR:
    report(EVT_INFO,"EvtGen") << "This is a scalar particle:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;     
  case EvtSpinType::VECTOR:
    report(EVT_INFO,"EvtGen") << "This is a vector particle:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;     
  case EvtSpinType::TENSOR:
    report(EVT_INFO,"EvtGen") << "This is a tensor particle:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;
  case EvtSpinType::DIRAC:
    report(EVT_INFO,"EvtGen") << "This is a dirac particle:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;
  case EvtSpinType::PHOTON:
    report(EVT_INFO,"EvtGen") << "This is a photon:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;
  case EvtSpinType::NEUTRINO:
    report(EVT_INFO,"EvtGen") << "This is a neutrino:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;
  case EvtSpinType::STRING:
    report(EVT_INFO,"EvtGen") << "This is a string:"<<EvtPDL::name(_id).c_str()<<"\n";
    break;
  default:
    report(EVT_INFO,"EvtGen") <<"Unknown particle type in EvtParticle::printParticle()"<<endl;
    break;
  }
  report(EVT_INFO,"EvtGen") << "Number of daughters:"<<_ndaug<<"\n";
...
double EvtParticle::initializePhaseSpace(
  ...
  if ( numdaughter == this->getNDaug() ) 
    for (i=0; i<numdaughter;i++) {
      ...
      //report(EVT_INFO,"EvtGen") << EvtPDL::name(this->getDaug(i)->getId()) 
  ...
  if ( poleSize<-0.1) {
    ...
    if ( numdaughter != 3 ) {
      report(EVT_ERROR,"EvtGen") << "Only can generate pole phase space "
    ...
    if ( !ok) {
      report(EVT_ERROR,"EvtGen") << "Invalid pair of particle to generate a pole dist "
			     << whichTwo1 << " " << whichTwo2
			     << endl<<"Will terminate."<<endl;
      ::abort();
    }
...
void EvtParticle::makeDaughters(unsigned int ndaugstore, std::vector<EvtId> idVector) {
  ...
  if (nVector < ndaugstore) {
    report(EVT_ERROR,"EvtGen") << "Asking to make "<<ndaugstore<<" daughters when there "
...
void EvtParticle::makeDaughters( unsigned int ndaugstore, EvtId *id){
  ...
  if (_ndaug!=0 ){
    if (_ndaug!=ndaugstore){
      report(EVT_ERROR,"EvtGen") << "Asking to make a different number of "
			     << "daughters than what was previously created."<<endl;
      report(EVT_ERROR,"EvtGen") << "Original parent:"<<EvtPDL::name(_id)<<endl;
      for (size_t i=0;i<_ndaug;i++){
	  report(EVT_ERROR,"EvtGen") << "Original daugther:"<<EvtPDL::name(getDaug(i)->getId())<<endl;
      }
      for (size_t i=0;i<ndaugstore;i++){
	  report(EVT_ERROR,"EvtGen") << "New Daug:"<<EvtPDL::name(id[i])<<endl;
      }
      report(EVT_ERROR,"EvtGen") << "Will terminate."<<endl;
      ::abort();
    }
  } 
  else{
