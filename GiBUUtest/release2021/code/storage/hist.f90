...
contains
  ...
  subroutine WriteHeader(H, iFile, mul)
    ...
    character(60), parameter :: fmt1000 = &
         "('###',/,'### Histogram: ',A,/,'###')"
    character(300),parameter :: fmt1010 = &
         &"('### Underflow: ',3ES11.4,/, &
         & '### Entries  : ',3ES11.4,/, &
         & '### Overflow : ',3ES11.4,/, &
         & '### Average  : ',3ES11.4,/, &
    ...
    ! write header
    write(iFile,fmt1010) &
         H%yVal(-1,1)*mulFak, H%yVal(-1,2), H%yVal(-1,3)*mulFak, &
...
!    real,dimension(1:3) :: S
  ...
  subroutine CopyHist(A, B)
    ...
    A%xMax = B%xMax
    A%xBin = B%xBin
    A%xExtreme = B%xExtreme
    A%Name = B%Name
    if (allocated(A%yVal)) deallocate(A%yVal)
    allocate(A%yVal(-1:ubound(B%yVal,1),3))
    A%yVal = B%yVal
