...
EvtTauolaEngine::EvtTauolaEngine(bool useEvtGenRandom) {
  ...
  report(EVT_INFO,"EvtGen")<<"Setting up TAUOLA."<<endl;
  ...
  if (useEvtGenRandom == true) {
    ...
    report(EVT_INFO,"EvtGen")<<"Using EvtGen random number engine also for Tauola++"<<endl;
...
void EvtTauolaEngine::setUpPossibleTauModes() {
  ...
  for (iPDL = 0; iPDL < nPDL; iPDL++) {
	...
	report(EVT_ERROR, "EvtGen") << "Please remove all non-TAUOLA decay modes for particle "
	...
	report(EVT_INFO,"EvtGen")<<"Setting TAUOLA BF modes using the definitions for the particle "
			     <<EvtPDL::name(particleId)<<endl;
	...
	for (iTauMode = 0; iTauMode < _nTauolaModes; iTauMode++) {
	  ...
	  report(EVT_INFO,"EvtGen")<<"Setting TAUOLA BF for mode "<<iTauMode+1<<" = "<<modeBF<<endl;
	  Tauolapp::Tauola::setTauBr(iTauMode+1, modeBF);
	  
	}

	report(EVT_INFO,"EvtGen")<<"Any other TAUOLA BF modes for other tau particle decay mode definitions will be ignored!"
			     <<endl;
