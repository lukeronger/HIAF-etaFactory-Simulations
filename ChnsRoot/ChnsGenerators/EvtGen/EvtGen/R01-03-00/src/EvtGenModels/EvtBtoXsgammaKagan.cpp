...
void EvtBtoXsgammaKagan::init(int nArg, double* args){
  ...
  report(EVT_ERROR,"EvtGen") << "EvtBtoXsgamma generator model "
  ...
  if (nArg>10){
    ...
    _mHmax = args[11]; 
    if (_mHmin > _mHmax){
      report(EVT_ERROR,"EvtGen") << "Minimum hadronic mass exceeds maximum " 
			     << endl;
      report(EVT_ERROR,"EvtGen") << "Will terminate execution!" << endl;
      ::abort();
    }
    if (_mHmin < mHminLimit){
      report(EVT_ERROR,"EvtGen") << "Minimum hadronic mass below K pi threshold" 
			     << endl;
      report(EVT_ERROR,"EvtGen") << "Resetting to K pi threshold" << endl;
      _mHmin = mHminLimit;
    }     
    if (_mHmax > mHmaxLimit){
      report(EVT_ERROR,"EvtGen") << "Maximum hadronic mass above 4.5 GeV/c^2" 
			     << endl;
      report(EVT_ERROR,"EvtGen") << "Resetting to 4.5 GeV/c^2" << endl;
      _mHmax = mHmaxLimit;
    }     
  }else{
...
void EvtBtoXsgammaKagan::computeHadronicMass(int /*nArg*/, double* args){
  ...
  report(EVT_WARNING,"EvtGen") << "EvtBtoXsgammaKagan: calculating new hadronic mass spectra. This takes a while..." << endl;
