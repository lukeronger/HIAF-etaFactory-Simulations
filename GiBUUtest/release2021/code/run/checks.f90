...
!   end subroutine ChecksSwitchGS
  ...
  subroutine evaluateFermiSurface(fileName)
    ...
    type(particle) :: p
    real :: rhoNuc,pot_p,pot_n, pf, average_fs, average,densInt,dp,fsInt
    real :: Ef_p,Ef_n,rhoNuc_p,rhoNuc_n,pf_p,pf_n,cpot,pot_p0,pot_n0
    type(dichte) :: dens
    ...
    write(100,'(A1,12A20)') &
         & "#","x", "E_f_p", "E_f_n", "pf_p", "pf_n",&
         & "Density_p", "Density_p", "pot_p", "pot_n", "pot_c",&
         & "pot_p(p=0)", "pot_n(p=0)"
...
!!$            (/0.,0./),(/0.4,0.18/),(/dp,0.005/))
       call CreateHist(momentumHist, '1/N dN/p^2 dp [GeV^(-3)]',0.,0.5,dp)
       DoInit = .false.
    end if
...
!!$          call AddHist2d(momDens,(/p,dens_nuc/) ,1.)
    ...
    !
    ! ttt gives the number of timesteps, ttt=000 describes initial groundstate
    ! Columns:
    ! * #1: p [GeV]
    ! * #2: n(p) [GeV^(-3)]
...
!!$       call removeHist2D(momDens)
  ...
  subroutine checkDensity(realParticles,filename)
    ...
    do i= lbound(realParticles,dim=1),ubound(realParticles,dim=1)
       do j= lbound(realParticles,dim=2),ubound(realParticles,dim=2)
          if (realParticles(i,j)%ID.ne.nucleon) cycle
          r = absPos(realParticles(i,j))
          call AddHist(positionHist,r ,1./r**2)
          integral = integral+1.
       end do
    end do

    !**************************************************************************
    !****o* checks/Density_ttt.dat
    ! NAME
    ! file Density_ttt.dat,   ttt=000 - number of timesteps
    ! PURPOSE
    ! This files provides the spatial density in the target
    !
    ! The momentum-distribution is normalized by the number of particles,
    ! i.e. the distribution given here is 4*pi*n(r)
    !
    ! ttt gives the number of timesteps, ttt=000 describes initial groundstate
    ! Columns:
    ! * #1: p [GeV]
    ...
    call WriteHist(positionHist,121,mul=1./integral)
    close(121)
