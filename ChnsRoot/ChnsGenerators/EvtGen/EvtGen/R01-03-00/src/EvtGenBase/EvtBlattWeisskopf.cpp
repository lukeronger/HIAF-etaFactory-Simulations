  ...
  : _LL(LL), _radial(R), _p0(p0)
{
  if(R < 0) {
    ...
    report(EVT_INFO,"EvtGen") << "Radius " << R << " negative" << endl;
    assert(0);
  }
  ...
  if(_F0 <= 0) {
    ...
    report(EVT_INFO,"EvtGen") << "Invalid nominal form factor computed " << _F0 << endl;
    assert(0);
  } 
}
...
double EvtBlattWeisskopf::operator()(double p) const
{
  ...
  //  report(EVT_INFO,"EvtGen") << p << " " << _p0 << " " << _F0 << " " << _LL << " " << _radial << " " << ret << endl;
  return ret;
}
