...
void EvtPartWave::init(){
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen")<<"_nA,_nB,_nC:"
			 <<_nA<<","<<_nB<<","<<_nC<<endl;
  }
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen")<<"_JA2,_JB2,_JC2:"
			 <<_JA2<<","<<_JB2<<","<<_JC2<<endl;
  }
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen")<<"Helicity states of particle A:"<<endl;
    for(i=0;i<_nA;i++){
      report(EVT_INFO,"EvtGen")<<_lambdaA2[i]<<endl;
    }

    report(EVT_INFO,"EvtGen")<<"Helicity states of particle B:"<<endl;
    for(i=0;i<_nB;i++){
      report(EVT_INFO,"EvtGen")<<_lambdaB2[i]<<endl;
    }

    report(EVT_INFO,"EvtGen")<<"Helicity states of particle C:"<<endl;
    for(i=0;i<_nC;i++){
      report(EVT_INFO,"EvtGen")<<_lambdaC2[i]<<endl;
    }

    report(EVT_INFO,"EvtGen")<<"Will now figure out the valid (M_LS) states:"<<endl;
  ...
  for (L=Lmin;L<=Lmax;L+=2){
    ...
    for (S=Smin;S<=Smax;S+=2){
      ...
      _nPartialWaveAmp++;
      if (verbose()){
	report(EVT_INFO,"EvtGen")<<"M["<<L<<"]["<<S<<"]"<<endl;    
      }
    }
  }
  ...
  for(i=0;i<_nPartialWaveAmp;i++){
    ...
    if (verbose()){
      report(EVT_INFO,"EvtGen")<<"M["<<_nL[i]<<"]["<<_nS[i]<<"]="<<_M[i]<<endl;
    }
  }
  ...
  for(ib=0;ib<_nB;ib++){
	...
	if (verbose()){
	  report(EVT_INFO,"EvtGen")<<"_HBC["<<ib<<"]["<<ic<<"]="<<_HBC[ib][ic]<<endl;
	}
      }
  ...
  if (fabs(helampsqtot-partampsqtot)/(helampsqtot+partampsqtot)>1e-6){
      report(EVT_ERROR,"EvtGen")<<"In EvtPartWave for decay "
			    << EvtPDL::name(getParentId()) << " -> "
                            << EvtPDL::name(getDaug(0)) << " "     
                            << EvtPDL::name(getDaug(1)) << std::endl; 
      report(EVT_ERROR,"EvtGen")<<"With arguments: "<<std::endl;
      for(i=0;i*2<getNArg();i++){
	  report(EVT_ERROR,"EvtGen") <<"M("<<_nL[i]<<","<<_nS[i]<<")="
//				 <<getArg(2*i)<<" "<<getArg(2*i+1)<<std::endl;
				 <<_M[i]<<std::endl;
      }
      report(EVT_ERROR,"EvtGen")<< "The total probability in the partwave basis is: "
			    << partampsqtot << std::endl;
      report(EVT_ERROR,"EvtGen")<< "The total probability in the helamp basis is: "
			    << helampsqtot << std::endl;
      report(EVT_ERROR,"EvtGen")<< "Most likely this is because the specified partwave amplitudes "
			    << std::endl;
      report(EVT_ERROR,"EvtGen")<< "project onto unphysical helicities of photons or neutrinos. "
			    << std::endl;
      report(EVT_ERROR,"EvtGen")<< "Seriously consider if your specified amplitudes are correct. "
			    << std::endl;
...
void EvtPartWave::initProbMax(){
  ...
  if (verbose()){
    report(EVT_INFO,"EvtGen")<<"Calculated probmax"<<maxprob<<endl;
  }
