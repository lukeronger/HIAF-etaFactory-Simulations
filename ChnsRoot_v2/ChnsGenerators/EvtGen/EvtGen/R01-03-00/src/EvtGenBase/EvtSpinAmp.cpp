...
{  
    ...
    if(_elem.size() != num ) {
        report(EVT_ERROR,"EvtGen")<<"Wrong number of elements input:"
...
{
    if( index.size()==0 ) {
        report(EVT_ERROR,"EvtGen") << "EvtSpinAmp can't handle no indices" << endl;
        ::abort();
    }

    if( index.size() != _twospin.size() ) {
        report(EVT_ERROR, "EvtGen" ) << "Rank of EvtSpinAmp index does not match: " 
    ...
    for( size_t i=0; i<_twospin.size(); ++i ) {
        if( static_cast<int>(_twospin[i])>=abs(index[i]) && static_cast<int>(_twospin[i])%2==index[i]%2 )
            continue; 
        report(EVT_ERROR,"EvtGen")<<"EvtSpinAmp index out of range" << endl;
        report(EVT_ERROR,"EvtGen")<<" Index: ";
        for(size_t j=0; j<_twospin.size(); ++j )
            report(EVT_ERROR," ")<<_twospin[j];

        report(EVT_ERROR, " ")<<endl<<" Index: ";
        for(size_t j=0; j<index.size(); ++j )
            report(EVT_ERROR," ")<<index[j];
...
EvtComplex & EvtSpinAmp::operator()( const vector<int>& index )
{
    ...
    if(trueindex >= _elem.size()) {
        report(EVT_ERROR,"EvtGen")<<"indexing error "<<trueindex<<" "<<_elem.size()<<endl;
        for(size_t i=0; i<_twospin.size(); ++i) {
            report(EVT_ERROR,"")<<_twospin[i]<<" ";
        }
        report(EVT_ERROR,"")<<endl;

        for(size_t i=0; i<index.size(); ++i) {
            report(EVT_ERROR,"")<<index[i]<<" ";
        }
        report(EVT_ERROR,"")<<endl;
