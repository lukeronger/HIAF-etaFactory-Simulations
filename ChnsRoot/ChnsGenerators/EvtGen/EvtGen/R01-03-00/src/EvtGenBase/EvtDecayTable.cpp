...
void EvtDecayTable::readDecayFile(const std::string dec_name, bool verbose){
  ...
  report(EVT_INFO,"EvtGen") << "In readDecayFile, reading:"<<dec_name.c_str()<<endl;
  
  ifstream fin;
  
  fin.open(dec_name.c_str());
  if (!fin) {
    report(EVT_ERROR,"EvtGen") << "Could not open "<<dec_name.c_str()<<endl;
  }
  fin.close();
  ...
  if (!hasend){
    report(EVT_ERROR,"EvtGen") << "Could not find an 'End' in "<<dec_name.c_str()<<endl;
  ...
  do{
	...
	report(EVT_INFO,"EvtGen") 
      ...
      if (id==EvtId(-1,-1)) {
	report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<oldname.c_str()
	...
	if (nameTemp!=";") {
      ...
      if (a==EvtId(-1,-1)) {
	report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<aname.c_str()
			       <<" on line "<<parser.getLineofToken(itoken)<<endl;
	report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
	::abort();
      }
      ...
      if (abar==EvtId(-1,-1)) {
	report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<abarname.c_str()
			       <<" on line "<<parser.getLineofToken(itoken)<<endl;
	report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
	::abort();
      }
	...
	command["GENERATOR"] = "Both";
      ...
      if (ipar==EvtId(-1,-1)) {
	report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<name.c_str()
			       <<" on line "
	...
	_decaytable[ipar.getAlias()].removeDecay();
      ...
      if ( verbose )
	report(EVT_INFO,"EvtGen") << pname.c_str() << endl;
      ...
      if (verbose )
	report(EVT_INFO,"EvtGen") << "Changing particle properties of " <<
      ...
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Refined minimum mass for " << EvtPDL::name(thisPart).c_str() << " to be " << tmass << endl;
      ...
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Refined maximum mass for " << EvtPDL::name(thisPart).c_str() << " to be " << tmass << endl;
      ...
      if ( verbose ) {
	if ( yesno ) report(EVT_DEBUG,"EvtGen") <<"Include birth factor for " << EvtPDL::name(thisPart).c_str() <<endl;
	if ( !yesno ) report(EVT_DEBUG,"EvtGen") <<"No longer include birth factor for " << EvtPDL::name(thisPart).c_str() <<endl;
      }
      ...
      if ( verbose ) {
	if ( yesno ) report(EVT_DEBUG,"EvtGen") <<"Include decay factor for " << EvtPDL::name(thisPart).c_str() <<endl;
	if ( !yesno ) report(EVT_DEBUG,"EvtGen") <<"No longer include decay factor for " << EvtPDL::name(thisPart).c_str() <<endl;
      }
      ...
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Change lineshape to non-rel BW for " << EvtPDL::name(thisPart).c_str() <<endl;
      ...
      EvtPDL::changeLS(thisPart,tstr);
      if (verbose) 
	report(EVT_DEBUG,"EvtGen") <<"Change lineshape to flat for " << EvtPDL::name(thisPart).c_str() <<endl;
      ...
      EvtPDL::changeLS(thisPart,tstr);
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Change lineshape to spikes for " << EvtPDL::name(thisPart).c_str() <<endl;
      ...
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Redefined Blatt-Weisskopf factor " << EvtPDL::name(thisPart).c_str() << " to be " << tnum << endl;
      ...
      if ( verbose )
	report(EVT_DEBUG,"EvtGen") <<"Redefined Blatt-Weisskopf birth factor " << EvtPDL::name(thisPart).c_str() << " to be " << tnum << endl;
      ...
      if ( verbose ) 
	report(EVT_DEBUG,"EvtGen") <<"Redefined Partial wave for " << pname.c_str() << " to " << pnameD1.c_str() << " " << pnameD2.c_str() << " ("<<pw<<")"<<endl;
      ...
      if (ipar==EvtId(-1,-1)) {
        report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<parent.c_str()
                               <<" on line "
                               <<parser.getLineofToken(itoken-1)<<endl;
        report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
        ::abort();
      ...
      if (_decaytable[ipar.getAlias()].getNMode()!=0) {
        report(EVT_DEBUG,"EvtGen") <<"Redefined decay of "
                               <<parent.c_str()<<endl;
        _decaytable[ipar.getAlias()].removeDecay();
      }
      ...
      do{
        ...
        if (token!="Enddecay"){
          ...
          while (token.c_str()[i++]!=0){
            if (isalpha(token.c_str()[i])){
              report(EVT_ERROR,"EvtGen") << 
                ...
                parser.getLineofToken(itoken-1)<<endl;
              report(EVT_ERROR,"EvtGen") << "Possibly to few arguments to model "<<
          ...
          if (!(isname||ismodel)){
            ...
            report(EVT_INFO,"EvtGen") << parser.getToken(itoken).c_str()
                                  << " is neither a particle name nor "
                                  << "the name of a model. "<<endl;
            report(EVT_INFO,"EvtGen") << "It was encountered on line "<<
              parser.getLineofToken(itoken)<<" of the decay file."<<endl;
            report(EVT_INFO,"EvtGen") << "Please fix it. Thank you."<<endl;
            report(EVT_INFO,"EvtGen") << "Be sure to check that the "
                                  << "correct case has been used. \n";
            report(EVT_INFO,"EvtGen") << "Terminating execution. \n";
            ::abort();
          ...
          while(EvtPDL::getId(parser.getToken(itoken)).getId()>=0){
            ...
            if (daught[n_daugh-1]==EvtId(-1,-1)) {
              report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<sdaug.c_str()
                                     <<" on line "<<parser.getLineofToken(itoken)<<endl;
              report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
              ::abort();
            }
	...
	report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<newname.c_str()
			       <<" on line "<<parser.getLineofToken(itoken)<<endl;
	report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
	::abort();
      }
      if (_decaytable[newipar.getAlias()].getNMode()!=0) {
	report(EVT_DEBUG,"EvtGen") <<"Redefining decay of "
			       <<newname<<endl;
    ...
    // Peter Onyisi, March 2008
      ...
      if (_decaytable[ipar.getAlias()].getNMode()==0) {
        report(EVT_DEBUG,"EvtGen") << "No decays to delete for "
                               << parent.c_str() << endl;
      } else {
        report(EVT_DEBUG,"EvtGen") <<"Deleting selected decays of "
                               <<parent.c_str()<<endl;
      }
      ...
      report(EVT_ERROR,"EvtGen") << "Found unknown command:'"<<token.c_str()<<"' on line "
			     <<parser.getLineofToken(itoken)<<endl;
      report(EVT_ERROR,"EvtGen") << "Will terminate execution!"<<endl;
      ::abort();
  ...
  for (size_t ii=0; ii<EvtPDL::entries(); ii++){
    ...
    if ( minMass > EvtPDL::getMinMass(temp) ) {
      if ( verbose )
	report(EVT_INFO,"EvtGen") << "Given allowed decays, resetting minMass " << EvtPDL::name(temp).c_str() << " " 
...
void EvtDecayTable::readXMLDecayFile(const std::string dec_name, bool verbose){
  ...
  while(parser.readNextTag()) {
      ...
      if(parser.getParentTagTitle() == "data") {
        if(parser.getTagTitle() == "photos") {
          ...
          if(mass != -1) {
            ...
            report(EVT_DEBUG,"EvtGen") <<"Refined mass for " << EvtPDL::name(thisPart).c_str() << " to be " << mass << endl;
          }
          if(width != -1) {
            EvtPDL::reSetWidth(thisPart, width);
            report(EVT_DEBUG,"EvtGen") <<"Refined width for " << EvtPDL::name(thisPart).c_str() << " to be " << width << endl;
          }
          if(minMass != -1) {
            EvtPDL::reSetMassMin(thisPart,minMass);
            report(EVT_DEBUG,"EvtGen") <<"Refined minimum mass for " << EvtPDL::name(thisPart).c_str() << " to be " << minMass << endl;
          }
          if(maxMass != -1) {
            EvtPDL::reSetMassMax(thisPart,maxMass);
            report(EVT_DEBUG,"EvtGen") <<"Refined maximum mass for " << EvtPDL::name(thisPart).c_str() << " to be " << maxMass << endl;
          }
          if(!birthFactor.empty()) {
            EvtPDL::includeBirthFactor(thisPart,stringToBoolean(birthFactor));
            if(verbose) {
              if(stringToBoolean(birthFactor)) {
                report(EVT_DEBUG,"EvtGen") <<"Include birth factor for " << EvtPDL::name(thisPart).c_str() <<endl;
              } else {
                report(EVT_DEBUG,"EvtGen") <<"No longer include birth factor for " << EvtPDL::name(thisPart).c_str() <<endl;
              }
            }
          }
          if(!decayFactor.empty()) {
            ...
            if(verbose) {
              if(stringToBoolean(decayFactor)) {
                report(EVT_DEBUG,"EvtGen") <<"Include decay factor for " << EvtPDL::name(thisPart).c_str() <<endl;
              } else {
                report(EVT_DEBUG,"EvtGen") <<"No longer include decay factor for " << EvtPDL::name(thisPart).c_str() <<endl;
              }
            }
          }
          if(!lineShape.empty()) {
            ...
            if ( verbose )
              report(EVT_DEBUG,"EvtGen") <<"Change lineshape to " << lineShape << " for " << EvtPDL::name(thisPart).c_str() <<endl;
          }
          if(blattWeisskopfD != -1) {
            EvtPDL::reSetBlatt(thisPart,blattWeisskopfD);
            if ( verbose )
              report(EVT_DEBUG,"EvtGen") <<"Redefined Blatt-Weisskopf factor "
                                     << EvtPDL::name(thisPart).c_str() << " to be " << blattWeisskopfD << endl;
          }
          if(blattWeisskopfB != -1) {
            EvtPDL::reSetBlattBirth(thisPart,blattWeisskopfB);
            if ( verbose )
              report(EVT_DEBUG,"EvtGen") <<"Redefined Blatt-Weisskopf birth factor "
          ...
          EvtPDL::setPWForBirthL(thisD2,pw,thisPart,thisD1);
          if ( verbose )
            report(EVT_DEBUG,"EvtGen") <<"Redefined Partial wave for " << parent.c_str() << " to "
        ...
        } else {  report(EVT_INFO,"EvtGen") << "Unknown tag "<<parser.getTagTitle()
      ...
      //TAGS FOUND UNDER DECAY
        ...
        if(parser.getTagTitle() == "channel") { //start of a channel
          ...
          _decaytable[ipar.getAlias()].finalize();
        } else report(EVT_INFO,"EvtGen") << "Unexpected tag "<<parser.getTagTitle()
                                     <<" found in XML decay file near line "<<parser.getLineNumber()<<". Tag will be ignored."<<endl;
      //TAGS FOUND UNDER REMOVEDECAY
        ...
        if(parser.getTagTitle() == "channel") { //start of a channel
          ...
          report(EVT_INFO,"EvtGen") << "Unexpected tag "<<parser.getTagTitle()
                                <<" found in XML decay file near line "<<parser.getLineNumber()<<". Tag will be ignored."<<endl;
        }
      }
  }//while lines in file
  ...
  if(!endReached) {
    report(EVT_INFO,"EvtGen") << "Either the decay file ended prematurely or the file is badly formed.\n"
...
void EvtDecayTable::checkParticle(std::string particle) {
  if (EvtPDL::getId(particle)==EvtId(-1,-1)) {
    report(EVT_ERROR,"EvtGen") <<"Unknown particle name:"<<particle.c_str()<<endl;
    report(EVT_ERROR,"EvtGen") <<"Will terminate execution!"<<endl;
    ::abort();
  }
}
