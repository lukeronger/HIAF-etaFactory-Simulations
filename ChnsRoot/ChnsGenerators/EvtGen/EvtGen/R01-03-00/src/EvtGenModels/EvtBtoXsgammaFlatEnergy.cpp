...
void EvtBtoXsgammaFlatEnergy::init(int nArg, double* args){
  ...
  if (_eMax>(pow(_mB0,2)-pow(mPi+mK,2))/(2.0*_mB0)){
    report(EVT_ERROR,"EvtGen") << "Emax greater than Kinematic limit" << endl;
    report(EVT_ERROR,"EvtGen") << "Reset to the kinematic limit" << endl;
    report(EVT_ERROR,"EvtGen") << "(m_B**2-(m_pi+m_k)**2)/(2m_B)" << endl;
    _eMax = (pow(_mB0,2)-pow(mPi+mK,2))/(2.0*_mB0);
  }
  _eRange=_eMax-_eMin;
}
