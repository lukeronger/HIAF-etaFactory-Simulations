...
TFile file("EtaDalitz_EpEmGamma.root");
tree->Draw("M2EpG : M2EmG >>h2(50,-0.05,0.34, 50,-0.05,0.34)","","colz");


	gPad->SetLogz();
	h2->SetTitle("");
	h2->GetXaxis()->SetTitle("M^{2}(e^{-}#gamma) / GeV^{2}");
	h2->GetXaxis()->SetTitleSize(0.06);
	h2->GetXaxis()->CenterTitle();
	h2->GetXaxis()->SetTitleOffset(1.05);
	h2->GetXaxis()->SetLabelSize(0.06);
	h2->GetYaxis()->SetTitle("M^{2}(e^{+}#gamma) / GeV^{2}");
