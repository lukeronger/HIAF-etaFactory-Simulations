...
void analyze_pdTo3HeEta_3piChannel()
{
	...
	/// The output file:
	TString output_file = "../../../HIAF-etaFactory-Simulations-localData/ChnsRoot-data/HIAF-pdTo3HeEta-3piChannel-part1.root";
	...
	TH1D * h3He_theta_MC = new TH1D("h3He_theta_MC","",100,0,180);
	TH1D * h3He_theta = new TH1D("h3He_theta","",100,0,180);
	...
	for(Int_t j=0; j<num;j++)
	{
		...
		for (Int_t i1=0; i1<fMC->GetEntries(); i1++){
			...
			if(pdg==-211) eta_Ep_MC.push_back(iMC->Get4Momentum());
			if(pdg==211)  eta_Em_MC.push_back(iMC->Get4Momentum());
			if(pdg==1000020030)  he3_MC.push_back(iMC->Get4Momentum());
		}
		...
		for (Int_t i1=0; i1<fCands->GetEntriesFast(); i1++)
		{
			...
			if(pdg==-211 && iCand1_prob->GetElectronPidProb()>0.1 && iCand1->GetCharge()>0) {
//				cout<<"e^+ find,    ";
			...
			else if(pdg==211 && iCand1_prob->GetElectronPidProb()>0.1 && iCand1->GetCharge()<0) {
//				cout<<"e^- find,    ";
...
//				cout<<"e^- find,    ";
				TLorentzVector _he3 = iCand1->GetLorentzVector();
				//em.SetE( sqrt(em.E()*em.E() - em.M()*em.M() + 0.000511*0.000511)  );
				he3.push_back(_he3);
				//Nem_selected++;
			}
...
//		cout<<"Nneutral_tracks="<<fNeuts->GetEntriesFast()<<endl;
		...
		h3He_theta_MC->Fill(he3_MC.at(0).Theta() * TMath::RadToDeg() );
		/// filling reconstructed eta distributions
		if(he3.size()>=1)
			h3He_theta->Fill(he3.at(0).Theta() * TMath::RadToDeg() );
		if(eta_Ep.size()>=1 && eta_Em.size()>=1 && eta_gamma.size()>=1 && he3.size()>=1){
