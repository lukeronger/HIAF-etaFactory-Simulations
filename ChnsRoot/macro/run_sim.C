void run_sim(Int_t nEvents = 10, TString mcEngine = "TGeant4", Bool_t isMT = false)
{
    ...
    FairDetector* tpc = new ChnsTpc("ChnsTpc", kTRUE);
    tpc->SetGeometryFileName("TPC_V1.1.root");
    run->AddModule(tpc);

    FairDetector* mvd = new ChnsMvd("ChnsMvd", kTRUE);
    run->AddModule(mvd);

    FairDetector* tof = new ChnsTof("ChnsTof", kTRUE);
    run->AddModule(tof);

    FairDetector* zdc = new ChnsZdc("ChnsZdc", kTRUE);
    run->AddModule(zdc);
    // ------------------------------------------------------------------------
    ...
    if(UseBoxGenerator){
	    ...
	    boxGen->SetThetaRange(80., 90.);                           // Polar angle in lab system range [degree]
