...
double EvtSpinDensity::normalizedProb(const EvtSpinDensity& d){
  ...
  if (dim!=d.dim) {
    report(EVT_ERROR,"EvtGen")<<"Not matching dimensions in NormalizedProb"<<endl;
    ::abort();
  }
  ...
  if (imag(prob)>0.00000001*real(prob)) {
    report(EVT_ERROR,"EvtGen")<<"Imaginary probability:"<<prob<<" "<<norm<<endl;
  }
  if (real(prob)<0.0) {
    report(EVT_ERROR,"EvtGen")<<"Negative probability:"<<prob<<" "<<norm<<endl;
  }
...
int EvtSpinDensity::check(){
  ...
  if (dim<1) {
    report(EVT_ERROR,"EvtGen")<<"dim="<<dim<<"in SpinDensity::Check"<<endl;
  }
  ...
  for(i=0;i<dim;i++){
    ...
    if (imag(rho[i][i])*1000000.0>trace) {
      report(EVT_INFO,"EvtGen") << *this << endl;
      report(EVT_INFO,"EvtGen") << trace << endl;
      report(EVT_INFO,"EvtGen") << "Failing 1"<<endl;
      return 0;
    }
  }
  ...
  for(i=0;i<dim;i++){
	...
	report(EVT_INFO,"EvtGen") << "Failing 2"<<endl;
	return 0;
      }
      if (fabs(imag(rho[i][j]+rho[j][i]))>
	  0.00000001*(abs(rho[i][i])+abs(rho[j][j]))) {
	report(EVT_INFO,"EvtGen") << "Failing 3"<<endl;
	return 0;
      }
    }
  }
