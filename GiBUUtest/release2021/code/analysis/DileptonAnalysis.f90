...
module Dilepton_Analysis
  ...
  ! * 4 = as 3, but times correction factor (for pn), see arXiv:2009.11702
  !****************************************************************************
...
contains
  ...
  real function dGamma_dM_omegaDalitz(m_w,M)
    use constants, only: pi,alphaQED,mPi,melec
...
!         call eNev_Set_PhiLepton(evt, phiL)  ! possibly rotate by angle phi
  ...
  !****************************************************************************
  !****s* Dilepton_Analysis/Dilep_write_CS_time
  ! NAME
  ! subroutine Dilep_write_CS_time
  ! PURPOSE
  ! Writes out the cross section spectra at the given time step.
  ! INPUTS
  ! OUTPUT
  ! The spectra are written to files called
  ! 'DileptonMass.dat', 'DileptonPlab.dat', etc
  ...
  subroutine Dilep_Brems (part, srtfree, XS_elast, XS_tot)
    ...
    if (first) then
      ...
      if(flagPrintBrems.and.success) then
         ...
         write(1,*)'# Ebeam, GeV:               M_ee, GeV:               dsigma/dM_ee,  mub/GeV:'
         do j=lbound(OBE_pp%entry,2),ubound(OBE_pp%entry,2)
            Eb=(j-lbound(OBE_pp%entry,2))*OBE_pp%delta(2)+OBE_pp%min(2)
            do i=lbound(OBE_pp%entry,1),ubound(OBE_pp%entry,1)
               mass=(i-lbound(OBE_pp%entry,1))*OBE_pp%delta(1)+OBE_pp%min(1)
               write(1,*) Eb,mass,OBE_pp%entry(i,j)
            end do
         end do
         close(1)
         open(1,file='OBE_pn.tst',status='unknown')
         write(1,*)'# min_mass, max_mass, delta_mass : ', OBE_pn%min(1), OBE_pn%max(1), OBE_pn%delta(1)
         write(1,*)'# min_Ebeam, max_Ebeam, delta_Ebeam : ', OBE_pn%min(2), OBE_pn%max(2), OBE_pn%delta(2)
         write(1,*)'# Ebeam, GeV:               M_ee, GeV:               dsigma/dM_ee,  mub/GeV:'
         do j=lbound(OBE_pn%entry,2),ubound(OBE_pn%entry,2)
            Eb=(j-lbound(OBE_pn%entry,2))*OBE_pn%delta(2)+OBE_pn%min(2)
            do i=lbound(OBE_pn%entry,1),ubound(OBE_pn%entry,1)
               mass=(i-lbound(OBE_pn%entry,1))*OBE_pn%delta(1)+OBE_pn%min(1)
    ...
    do k=1,nevent ! loop to enhance statistics
      ...
      if (brems.ge.2 .and. (ch==11 .or. ch==12)) then
        ...
        M = rnFlat(M_min,M_max)                    ! dilepton mass
        q0max = (s+M**2-(m1+m2)**2) / (2*srtfree)  ! max. energy
        q0 = rnFlat(M,q0max)                       ! dilepton energy

        s2 = s+M**2-2.*srtfree*q0  ! energy of two-nucleon system after radiation > (m1+m2)**2

        q = sqrt(q0**2-M**2)           ! dilepton momentum

!**** Expressions from Gy. Wolf et al., NPA 517 (1990) 615 ***************
...
!*************************************************************************
  ...
  contains
    ...
    real function fcor(Mee)
      ...
      real, parameter :: C=1.5, d=0.01, a=0.10, b=0.55, w=3.0
