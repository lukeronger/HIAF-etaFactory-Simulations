...
double EvtBtoXsgammaFermiUtil::FermiExpFunc(double y, const std::vector<double> &coeffs) {
  ...
  // report(EVT_INFO,"EvtGen")<<coeffs[4]<<endl;
...
double EvtBtoXsgammaFermiUtil::BesselK1(double x) {
  ...
  if (x<0.0) report(EVT_INFO,"EvtGen") <<"x is negative !"<<endl;
...
double EvtBtoXsgammaFermiUtil::FermiRomanFuncRoot(double lambdabar, double lam1) {
  ...
  //rho=0.250353;
  report(EVT_INFO,"EvtGen")<<"rho/2 "<<rho/2.<<" bessel "<<BesselK1(rho/2.)<<endl;
  double pF = lambdabar*sqrt(EvtConst::pi)/(rho*exp(rho/2.)*BesselK1(rho/2.));
  report(EVT_INFO,"EvtGen")<<"rho "<<rho<<" pf "<<pF<<endl;
...
double EvtBtoXsgammaFermiUtil::FermiRomanFunc(double y, const std::vector<double> &coeffs) {
  ...
  //  report(EVT_INFO,"EvtGen")<<" pf "<<y<<" "<<pF<<" "<<coeffs[1]<<" "<<coeffs[2]<<" "<<coeffs[3]<<" "<<coeffs[4]<<" "<<coeffs[5]<<endl;
  //double pF=0.382533;

  //report(EVT_INFO,"EvtGen")<<(coeffs[1]-coeffs[2])*(1./(sqrt(EvtConst::pi)*pF))<<endl;
  //report(EVT_INFO,"EvtGen")<<(1.-y/(coeffs[1]-coeffs[2]))<<endl;
  //report(EVT_INFO,"EvtGen")<<(coeffs[1]-coeffs[2])<<endl;
  //report(EVT_INFO,"EvtGen")<<(coeffs[1]-coeffs[2])*(1.-y/(coeffs[1]-coeffs[2]))<<endl;

  //report(EVT_INFO,"EvtGen")<<" "<<pF*coeffs[3]/((coeffs[1]-coeffs[2])*(1.-y/(coeffs[1]-coeffs[2])))<<endl;
  // report(EVT_INFO,"EvtGen")<<" "<<((coeffs[1]-coeffs[2])/pF)*(1. -y/(coeffs[1]-coeffs[2]))<<endl;

  //report(EVT_INFO,"EvtGen")<<"result "<<(coeffs[1]-coeffs[2])*(1./(sqrt(EvtConst::pi)*pF))*exp(-(1./4.)*pow(pF*(coeffs[3]/((coeffs[1]-coeffs[2])*(1.-y/(coeffs[1]-coeffs[2])))) - ((coeffs[1]-coeffs[2])/pF)*(1. -y/(coeffs[1]-coeffs[2])),2.))/coeffs[5];

  //report(EVT_INFO,"EvtGen")<<"leaving"<<endl;
