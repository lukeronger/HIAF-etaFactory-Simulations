...
TLorentzVector *eta, *eta_3pidecayPip, *eta_3pidecayPim, *eta_3pidecayPi0;
TLorentzVector *eta_3pidecayGamma1, *eta_3pidecayGamma2, *eta_dimugammadecayGamma;
TLorentzVector *eta_dimugammadecayMup, *eta_dimugammadecayMum;
TLorentzVector *eta_dielegammadecayGamma, *eta_dielegammadecayEp, *eta_dielegammadecayEm;
...
int initialization(){
	...
	eta = new TLorentzVector(0, 0, 0, 0);
	eta_3pidecayPip = new TLorentzVector(0, 0, 0, 0);
	eta_3pidecayPim = new TLorentzVector(0, 0, 0, 0);
	eta_3pidecayPi0 = new TLorentzVector(0, 0, 0, 0);
	eta_3pidecayGamma1 = new TLorentzVector(0, 0, 0, 0);
	eta_3pidecayGamma2 = new TLorentzVector(0, 0, 0, 0);
	eta_dimugammadecayGamma = new TLorentzVector(0, 0, 0, 0);
	eta_dimugammadecayMup = new TLorentzVector(0, 0, 0, 0);
	eta_dimugammadecayMum = new TLorentzVector(0, 0, 0, 0);
	eta_dielegammadecayGamma = new TLorentzVector(0, 0, 0, 0);
	eta_dielegammadecayEp = new TLorentzVector(0, 0, 0, 0);
	eta_dielegammadecayEm = new TLorentzVector(0, 0, 0, 0);
...
/*	outtree->Branch("eventNO",&eventNO,"eventNO/L");
	outtree->Branch("eta",eta,32000,0);
	outtree->Branch("eta_3pidecayPip",eta_3pidecayPip,32000,0);
	outtree->Branch("eta_3pidecayPim",eta_3pidecayPim,32000,0);
	outtree->Branch("eta_3pidecayPi0",eta_3pidecayPi0,32000,0);
	outtree->Branch("eta_3pidecayGamma1",eta_3pidecayGamma1,32000,0);
	outtree->Branch("eta_3pidecayGamma2",eta_3pidecayGamma2,32000,0);
	outtree->Branch("eta_dimugammadecayGamma",eta_dimugammadecayGamma,32000,0);
	outtree->Branch("eta_dimugammadecayMup",eta_dimugammadecayMup,32000,0);
	outtree->Branch("eta_dimugammadecayMum",eta_dimugammadecayMum,32000,0); */
	outtree->Branch("eventNO",&eventNO,"eventNO/L");
	outtree->Branch("eta","TLorentzVector",eta);
	outtree->Branch("eta_3piPip","TLorentzVector",eta_3pidecayPip);
	outtree->Branch("eta_3piPim","TLorentzVector",eta_3pidecayPim);
	outtree->Branch("eta_3piPi0","TLorentzVector",eta_3pidecayPi0);
	outtree->Branch("eta_3piPi0Gamma1","TLorentzVector",eta_3pidecayGamma1);
	outtree->Branch("eta_3piPi0Gamma2","TLorentzVector",eta_3pidecayGamma2);
	outtree->Branch("eta_MuMuGGamma","TLorentzVector",eta_dimugammadecayGamma);
	outtree->Branch("eta_MuMuGMup","TLorentzVector",eta_dimugammadecayMup);
	outtree->Branch("eta_MuMuGMum","TLorentzVector",eta_dimugammadecayMum);
	outtree->Branch("eta_EEGGamma","TLorentzVector",eta_dielegammadecayGamma);
	outtree->Branch("eta_EEGEp","TLorentzVector",eta_dielegammadecayEp);
	outtree->Branch("eta_EEGEm","TLorentzVector",eta_dielegammadecayEm);
...
int etaDecayDielectronGamma(double _px, double _py, double _pz, double _E){
	...
	mass = 0; // gamma->M();
	pid = 50*100 + 0;
	*eta_dielegammadecayGamma = *gamma;
	//rootfile->cd();
	//outtree->Fill();
	...
	pid = 52*100 + 0;
	*eta_dielegammadecayEp = *electron1;
	//rootfile->cd();
	//outtree->Fill();
	px = electron2->Px();
	py = electron2->Py();
	...
	pid = 51*100 + 0;
	*eta_dielegammadecayEm = *electron2;
	//rootfile->cd();
	//outtree->Fill();
...
//////////////////////////////////////////////////////////////////////////////
int BUUEta2root(string filename="IBUUk.mom", double Ebeam=9){
	initialization();
	...
	while(1){
		...
		for(int i=0;i<multi;i++){
			if(_pid[i]==4){
				...
				mass = particleMass(pid);
				E = sqrt(px*px+py*py+pz*pz+mass*mass);
				eta->SetXYZT(px, py, pz, E);
				//// get rid of the abnormal events
				if(eta->P()>Ebeam*(3.17-eta->Theta())/3.17)continue;
				//rootfile->cd();
				//outtree->Fill();
				//etaDecayDimuonGamma(px, py, pz, E);
				double etapx = px;
				double etapy = py;
				double etapz = pz;
				double etaE = E;
				etaDecay(etapx, etapy, etapz, etaE);
				etaDecayDimuonGamma(etapx, etapy, etapz, etaE);
				etaDecayDielectronGamma(etapx, etapy, etapz, etaE);
				rootfile->cd();
				outtree->Fill();
			}
			else{
