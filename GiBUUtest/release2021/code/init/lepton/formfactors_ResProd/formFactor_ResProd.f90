 ...
 !   use readresparam, only: MADelta
    ...
    if (HNV_axialFF) then
       if (FF_ResProd.eq.1) then
          ...
          write(*,*) 'NOTE: HNV_axialFF is set to true but has no effect for FF_ResProd.ne.1', &
                     & FF_ResProd
       end if
    end if
  ...
  contains
    ...
    subroutine MAID_CM(C3,C4,C5,Qs,resID,FF_set)
      ...
      case (S11_1535,S31_1620,S11_1650)
         ...
         F1=(2*mn**2*(Sqrt(2.)*A12*Sqrt(-((alphaQED*((mn - ms)**2 - q2)*((mn + ms)**2  &
              &- q2)**2)/ &
         ...
         F2=-((Sqrt(2.)*mn*(A12*(mn - ms)*Sqrt(-((alphaQED*((mn - ms)**2 - q2)*((mn + ms)**2 - &
              ...
              & Sqrt(-((alphaQED*((mn - ms)**2 - q2)*((mn + ms)**2 - q2)**2)  &
              & /(mn*(mn - ms)*ms**2 &
  ...
  contains
    ...
    subroutine MAIDLala(C3,C4,C5,Qs,resID,FF_set)
      ...
      case (P11_1440,P31_1910)
         ...
         C3=(mu**2*(MR + ppr0)*(A12*qz + sqrt2*(mN + MR)*S12))/  &
            &  (sqrt2*sqrtN*((mN + MR)**2 + Qs)*qz**2)

         C4=(mu*(MR + ppr0)*(A12*(mN + MR)*qz - sqrt2*Qs*S12))/  &
            & (sqrt2*sqrtN*((mN + MR)**2 + Qs)*qz**2)
      ...
      case (D13_1520,D33_1700,D15_1675)
         ...
         C4=(mN*(3*sqrt2*A12*MR*(MR + ppr0)*(mN*(mN - MR)*(mN + q0) + MR*qtimesp) -          &
              ...
              &  (2.*Sqrt(6.)*sqrtN*((-(mN*(mN + 2*q0)) + Qs)*qtimesp   &
              &  + mN*(mN + q0)*qtimesppr)*qz**2)
