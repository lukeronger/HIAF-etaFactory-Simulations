...
int run_fsim_BoxGen(int nevts=0, int seed_num = 1000)
{
  TStopwatch timer;
  timer.Start();
  gDebug=0;
  ...
  TString outfile;
  if (outfile=="") {
	outfile="run_fsim_BoxGen"; //1.root";
  	outfile+= seed_num;
	outfile+="_EMCtest.root";
  }
...
/*
  ...
  boxGen1->SetPDGType(22);  
  boxGen1->SetThetaRange(5,178);
  boxGen1->SetPhiRange(0, 360);
  boxGen1->SetPRange(0.1, 15);
  ...
  boxGen3->SetPDGType(2112);  //neutron
