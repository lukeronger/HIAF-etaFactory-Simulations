...
module inputGeneral
  ...
  ! SOURCE
  !
  character(1000), save :: path_To_Input = '../../buuinput'
  !****************************************************************************
  ...
  !****************************************************************************
  !****g* inputGeneral/PrintCollisionList
  ! PURPOSE
  ! Switch to turn on the printing of the particles and the collisions
  ! continously during the run
  ! SOURCE
  !
  logical, save :: PrintCollisionList=.false.
  !****************************************************************************
...
contains
  ...
  subroutine readInputGeneral
    ...
    use eventTypes, only: HeavyIon, Neutrino, HiPion, HiLepton
    use CallStack, only: Traceback
    use particleDefinition, only: useProductionPos
    ...
    NAMELIST /input/ path_To_Input, numEnsembles, eventtype, &
         ...
         printParticleVectors, printParticleVectorTime, &
    ...
    if (useProductionPos) then
       ...
       case (HiLepton)
          ! do nothing
       case default
          call Traceback("eventType not prepared to use together with production pos.!")
       end select
    end if
