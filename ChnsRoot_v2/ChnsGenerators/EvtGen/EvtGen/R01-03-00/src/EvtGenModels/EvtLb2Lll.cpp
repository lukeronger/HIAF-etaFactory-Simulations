...
void EvtLb2Lll::init(){
  ...
  if(getNArg()>8){ // Decay parameters
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected max. 8 arguments but found: " << getNArg() << std::endl;
    report(EVT_INFO,"EvtGen") << "  1. Lambda_b0 polarization - zero is default" << std::endl;
    report(EVT_INFO,"EvtGen") << "  2. Model type - \"SM\" for Standard Model is default" << std::endl;
    report(EVT_INFO,"EvtGen") << "  3. Form-Factors - \"HQET\" is used by default" << std::endl;
    report(EVT_INFO,"EvtGen") << "  4. How to set polarization - \"ModifiedSpinors\" is default" << std::endl;
    report(EVT_INFO,"EvtGen") << "  5. Include long distance (LD) effects - \"SD\" (no) is default" << std::endl;
    report(EVT_INFO,"EvtGen") << "  6. NonFactorizable contribution (omega) to b->sg decay at q2=0 " << std::endl;
    report(EVT_INFO,"EvtGen") << "  7. Note on every x-th decay" << std::endl;
    report(EVT_INFO,"EvtGen") << "  8. Maximum probability - automatic by default" << std::endl;
    ::abort();
  }

  if(getNDaug()!=3){ // Check that there are 3 daughters only
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected 3 daughters but found: " << getNDaug() << std::endl;
    ::abort();
  }
  ...
  if(EvtPDL::name(getParentId())=="Lambda_b0"){ // Check daughters of Lambda_b0
    report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found Lambda_b0" << std::endl;
    //if(EvtPDL::name(getDaug(0))!="Lambda0"){
    //  report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected Lambda0 daughter but found: " << EvtPDL::name(getDaug(0)) << std::endl;
    //  ::abort();
    //}
    if(EvtPDL::name(getDaug(1))=="e-" && EvtPDL::name(getDaug(2))=="e+"){
      m_decayName="Lambda_b0 -> Lambda0 e- e+";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  Lambda_b0 -> Lambda0 e- e+" << std::endl;
    }else if(EvtPDL::name(getDaug(1))=="mu-" && EvtPDL::name(getDaug(2))=="mu+"){
      m_decayName="Lambda_b0 -> Lambda0 mu- mu+";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  Lambda_b0 -> Lambda0 mu- mu+" << std::endl;
    }else if(EvtPDL::name(getDaug(1))=="tau-" && EvtPDL::name(getDaug(2))=="tau+"){
      m_decayName="Lambda_b0 -> Lambda0 tau- tau+";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  Lambda_b0 -> Lambda0 tau- tau+" << std::endl;
    }else{
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected lepton pair daughters but found:  " << EvtPDL::name(getDaug(1)) << " " << EvtPDL::name(getDaug(2)) << std::endl;
      ::abort();
    }
  //TODO: The model is known not to work correctly for anti-Lambda_b0 (A_FB does not change its sign)
    ...
    report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found anti-Lambda_b0" << std::endl;
    //if(EvtPDL::name(getDaug(0))!="anti-Lambda0"){
    //  report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected anti-Lambda0 daughter but found: " << EvtPDL::name(getDaug(0)) << std::endl;
    //  ::abort();
    //}
    if(EvtPDL::name(getDaug(1))=="e+" && EvtPDL::name(getDaug(2))=="e-"){
      m_decayName="anti-Lambda_b0 -> anti-Lambda0 e+ e-";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  anti-Lambda_b0 -> anti-Lambda0 e+ e-" << std::endl;
    }else if(EvtPDL::name(getDaug(1))=="mu+" && EvtPDL::name(getDaug(2))=="mu-"){
      m_decayName="anti-Lambda_b0 -> anti-Lambda0 mu+ mu-";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  anti-Lambda_b0 -> anti-Lambda0 mu+ mu-" << std::endl;
    }else if(EvtPDL::name(getDaug(1))=="tau-" && EvtPDL::name(getDaug(2))=="tau+"){
      m_decayName="anti-Lambda_b0 -> anti-Lambda0 tau+ tau-";
      report(EVT_INFO,"EvtGen") << " EvtLb2Lll generator found decay:  anti-Lambda_b0 -> anti-Lambda0 tau+ tau-" << std::endl;
    }else{
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected lepton pair daughters but found:  " << EvtPDL::name(getDaug(1)) << " " << EvtPDL::name(getDaug(2)) << std::endl;
      ::abort();
    }
  }else{ // This model is not intended for decay of anything else than (anti-)Lambda_b0
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll generator expected (anti-)Lambda_b0 parent but found:  " << EvtPDL::name(getParentId()) << std::endl;
    ::abort();
  }
  ...
  if(getNArg()>0){
    if(getArg(0)>1. || getArg(0)<-1.){
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll expects polarization to be in interval <-1,1>, not " << getArg(0) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll set Lambda_b0 polarization to " << m_polarizationLambdab0 << std::endl;
  ...
  if(getNArg()>1){
    if(getArgStr(1).substr(1,getArgStr(1).size()-2)!="SM" &&
      ...
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll doesn't know this physics model: " << getArgStr(1) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll will use this physics model: " << m_HEPmodel << std::endl;
  ...
  if(getNArg()>2){
    if(getArgStr(2).substr(1,getArgStr(2).size()-2)!="HQET" &&
      ...
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll doesn't know this Form-Factors model: " << getArgStr(2) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll will use this Form-Factors model: " << m_FFtype << std::endl;

  if(getNArg()>3){
    if(getArgStr(3).substr(1,getArgStr(3).size()-2)!="Unpolarized"){
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll doesn't know kind of introducing polarization: " << getArgStr(3) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll will use this kind of introducing polarization: " << m_polarizationIntroduction << std::endl;

  if(getNArg()>4){
    if(getArgStr(4).substr(1,getArgStr(4).size()-2)!="SD" && getArgStr(4).substr(1,getArgStr(4).size()-2)!="LD"){
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll didn't find SD or LD parameter: " << getArgStr(4) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll will include contribution from these effects: " << m_effectContribution << std::endl;

  if(getNArg()>5){
    if(fabs(getArg(5))>0.15){
      report(EVT_WARNING,"EvtGen") << " WARNING: EvtLb2Lll found very high contribution to b->sg decay at q2=0: " << getArg(5) << std::endl;
    }
    m_omega = getArg(5);
  }else{
    m_omega = 0;
  }
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll will use this contribution to b->sg decay at q2=0: " << m_omega << std::endl;
  ...
  if(getNArg()>7){
    if(getArg(7)<0.){
      report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll expects positive maximum probability not : " << getArg(7) << std::endl;
      ::abort();
    }
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll maximum probability was set to " << m_maxProbability << std::endl;
  m_poleSize=0;
...
void EvtLb2Lll::initProbMax(){
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll is finding maximum probability ... " << std::endl;
  ...
  if(m_maxProbability==0){
    ...
    report(EVT_INFO,"EvtGen") << " EvtLb2Lll is probing whole phase space ..." << std::endl;
	...
	if(prob>m_maxProbability){
	  report(EVT_INFO,"EvtGen") << "  - probability " << prob << " found at q2 = " << q2 << " (" << 100*(q2-q2min)/(q2max-q2min) << " %) and theta = " << theta*180/EvtConst::pi << std::endl;
  ...
  report(EVT_INFO,"EvtGen") << " EvtLb2Lll set up maximum probability to " << m_maxProbability << std::endl;
...
void EvtLb2Lll::calcAmp(EvtAmp *amp,EvtParticle *parent){
  ...
  if(m_noTries>0) if(!((++noTries)%m_noTries)) report(EVT_DEBUG,"EvtGen") << " EvtLb2Lll already finished " << noTries << " matrix element calculations" << std::endl;
  ...
  if(m_FFtype=="HQET"){
    ...
    //report(EVT_WARNING,"EvtGen") << " WARNING: HQET-delta FF model should be checked for correctness" << std::endl;
    ...
    //report(EVT_WARNING,"EvtGen") << " WARNING: HQET-noF2 FF model should be checked for correctness" << std::endl;
    r = M_L*M_L/M_Lb/M_Lb;
    F0_1 = +0.462;
    a_F1 = -0.0182;
    b_F1 = -0.000176;
    ...
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll - unknown Form-Factors model: " << m_FFtype << " - this should never happen !" << std::endl;
    ::abort();
  }
  ...
  if(m_HEPmodel=="SM"){
    ...
    //report(EVT_WARNING,"EvtGen") << " WARNING: SUSY-ChenGeng model should be checked for correctness" << std::endl;
    C_LL=C_LR=C_RL=C_RR=C_LRLR=C_RLLR=C_LRRL=C_RLRL=C_T=C_TE=EvtComplex(0,0);
    ...
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll - unknown physics model: " << m_HEPmodel << " - this should never happen !" << std::endl;
    ::abort();
  }
  ...
  if(m_polarizationIntroduction=="SpinDensityMatrix"){
    ...
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtLb2Lll - unknown polarization: " << m_polarizationIntroduction << " - this should never happen !" << std::endl;
    ::abort();
  }
