...
#include "ChnsMCTrack.h"     // for ChnsMCTrack
...
// -----   Default constructor   -------------------------------------------
ChnsStack::ChnsStack(Int_t size)
    : FairGenericStack()
    , fStack()
    , fParticles(new TClonesArray("TParticle", size))
    , fTracks(new TClonesArray("ChnsMCTrack", size))
    , fStoreMap()
    , fStoreIter()
    , fIndexMap()
    , fIndexIter()
...
ChnsStack::ChnsStack(const ChnsStack& right)
    : FairGenericStack(right)
    , fStack()
    , fParticles(new TClonesArray("TParticle", 100))
    , fTracks(new TClonesArray("ChnsMCTrack", 100))
    , fStoreMap()
    , fStoreIter()
    , fIndexMap()
    , fIndexIter()
...
void ChnsStack::PushTrack(Int_t toBeDone,
                            Int_t parentId,
                            Int_t pdgCode,
                            Double_t px,
                            Double_t py,
...
TParticle* ChnsStack::PopNextTrack(Int_t& iTrack)
{
...
TParticle* ChnsStack::PopPrimaryForTracking(Int_t iPrim)
{
    ...
    if (iPrim < 0 || iPrim >= fNPrimaries) {
        LOG(fatal) << "ChnsStack: Primary index out of range! " << iPrim;
    }
    ...
    if (!(part->GetMother(0) < 0)) {
        LOG(fatal) << "ChnsStack:: Not a primary track! " << iPrim;
    }
...
TParticle* ChnsStack::GetCurrentTrack() const
{
    TParticle* currentPart = GetParticle(fCurrentTrack);
    if (!currentPart) {
        LOG(warning) << "ChnsStack: Current track not found in stack!";
    }
    return currentPart;
}
// -------------------------------------------------------------------------
...
void ChnsStack::AddParticle(TParticle* oldPart)
{
...
void ChnsStack::FillTrackArray()
{

    LOG(debug) << "ChnsStack: Filling MCTrack array...";
    ...
    for (Int_t iPart = 0; iPart < fNParticles; iPart++) {
        ...
        if (fStoreIter == fStoreMap.end()) {
            LOG(fatal) << "ChnsStack: Particle " << iPart << " not found in storage map! ";
        }
        Bool_t store = (*fStoreIter).second;

        if (store) {
            ChnsMCTrack* track = new ((*fTracks)[fNTracks]) ChnsMCTrack(GetParticle(iPart));
            fIndexMap[iPart] = fNTracks;
            // --> Set the number of points in the detectors for this track
            for (Int_t iDet = kChnsEmc; iDet < kSTOPHERE; iDet++) {
...
void ChnsStack::UpdateTrackIndex(TRefArray* detList)
{

    LOG(debug) << "ChnsStack: Updating track indizes...";
    Int_t nColl = 0;

    // First update mother ID in MCTracks
    for (Int_t i = 0; i < fNTracks; i++) {
        ChnsMCTrack* track = (ChnsMCTrack*)fTracks->At(i);
        Int_t iMotherOld = track->GetMotherId();
        fIndexIter = fIndexMap.find(iMotherOld);
        if (fIndexIter == fIndexMap.end()) {
            LOG(fatal) << "ChnsStack: Particle index " << iMotherOld << " not found in dex map! ";
        }
    ...
    while ((det = (FairDetector*)fDetIter->Next())) {
        ...
        while ((hitArray = det->GetCollection(iColl++))) {
            ...
            for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
                ...
                if (fIndexIter == fIndexMap.end()) {
                    LOG(fatal) << "ChnsStack: Particle index " << iTrack << " not found in index map! ";
                }
...
void ChnsStack::AddPoint(DetectorId detId)
{
    Int_t iDet = detId;
...
void ChnsStack::AddPoint(DetectorId detId, Int_t iTrack)
{
    if (iTrack < 0) {
        return;
    }
...
Int_t ChnsStack::GetCurrentParentTrackNumber() const
{
...
TParticle* ChnsStack::GetParticle(Int_t trackID) const
{
    if (trackID < 0 || trackID >= fNParticles) {
        LOG(fatal) << "ChnsStack: Particle index " << trackID << " out of range.";
    }
