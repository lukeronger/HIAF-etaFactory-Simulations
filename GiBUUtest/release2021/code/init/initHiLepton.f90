...
contains
  ...
  subroutine InitHiLeptonInduced(rParts,pParts,targetNuc)
    ...
    integer, parameter :: nOutPart = 200
...
!!$       call CreateHist(h_RhoTF, "t_F (Rho0)", 0.0,20.0,0.1)
    ...
    if (realRun) then
       ...
       pArr => pParts
       call InitResidue(nEns,nPart,targetNuc%mass,targetNuc%charge)
    end if
...
101       continue
          ...
          do
             ...
             if (iExperiment.eq.0) then
                ...
                eNev_RunData = photonFluxMC(target)
             end if
...
!!$       call writeFileDocu('initHiLep.RhoTF.dat','formation time')
  ...
  subroutine initExperiment
    ...
    real :: Q2max1,Q2max2,qsqmin,qsqmax
    real, dimension(0:3) :: pE,pA
    real, dimension(1:3) :: beta

    qsqmax = 99.9d0 ! some large default value
...
!       qsqmin = 0.0
    ...
    if (user_qsqmax>0) then
       write(*,'("OVERRIDE: ",A10," = ",1P,e13.5," --> ",e13.5)') "qsqmax",qsqmax,user_qsqmax
       qsqmax = user_qsqmax
    end if
    ...
    Q2max=min(Q2max1,Q2max2,qsqmax)
...
!       call write_electronNucleon_event(eNev, .false., .true.)
  ...
  real function AccWeight(iDetector,partID,partCharge,momentum,theta)
    ...
    use callstack, only: traceback
    use IDtable, only: pion, electron

    integer,intent(in) :: iDetector, partID, partCharge
...
!    real,parameter :: th1Ce=0.2443, th2Ce=0.8727 ! = 14°,50°
    ...
    case (4) ! ===== CLAS @ 5GeV
       ...
       select case (partID)
       case (electron)
          AccWeight = CLAS(0,momentum,theta)
       case (pion)
          select case (partCharge)
          case (-1,1)
             AccWeight = CLAS(partCharge,momentum,theta)
          case (0)
             AccWeight = CLASpi0(momentum,theta)
          end select
       case default
          AccWeight = 0
       end select
...
!c     0 to 1. It does not have to equal 4pi when integrated over.
    ...
    real FUNCTION CLASpi0(p,theta_rad)
      ...
      real :: theta
      logical, parameter :: onlyCuts = .false.

      theta=theta_rad*180./pi
      CLASpi0 = 0.0

      if (theta <  5.0) return
      if (theta > 45.0) return
      if (p < 0.5846) return
      if (p > 4.1) return
      ...
      if (onlyCuts) then
         ...
         ! This is a phenomenological fit to the theta integrated acceptance.
         ! The theta-dependence has shown to be quite smooth, therefore we
         ! use only the cuts.
         CLASpi0 = 1.0 * (p-0.554807)**1.34758 * p**(-1.30341)
      end if

    end FUNCTION CLASpi0
  ...
  ! * 3-momentum conservation is somehow peculiar. Therefore you may switch
  !   it on and off at compile time.
  !****************************************************************************
  logical function checkConservation(eNev,outPart,channel)
    ...
    logical, parameter :: doMom = .FALSE. ! see NOTES above
...
!    endif
    ...
    if (outSUM%charge .ne. eNev%nucleon_free%charge) then
       if (DoPr(2)) write(*,'(A,2i5)') 'Problem in initHiLepton: charge not conserved',&
