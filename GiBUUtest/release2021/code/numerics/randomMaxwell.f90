...
contains
  ...
  !
  ! it is indeed p^2*exp(-sqrt(m^2+p^2)/T), but the momentum is scaled to the
  ! mass. In addition, the function is transformed to sample the kinetic
  ! energy, exp(-E/T) = exp(-m/T)*exp(-(E-m)/T), and then avoiding numerical
  ! problems by rewriting (E-m)=(E-m)(E+m)/(E+m)=(E^2-m^2)/(E+m)=p^2/(E+m)
  !
