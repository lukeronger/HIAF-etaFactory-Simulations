...
void EvtBtoXsll::init(){
  ...
  if ( etyp != 2 && mutyp != 2 && tautyp != 2 ) {
     ...
     report(EVT_ERROR,"EvtGen") << "Expect two leptons of the same type in EvtBtoXsll.cc\n";
     ::abort();
  }
  ...
  if ( lpos != 1 || lneg != 1 ) {
     ...
     report(EVT_ERROR,"EvtGen") << "Expect 2nd and 3rd particles to be positive and negative leptons in EvtBtoXsll.cc\n";
     ::abort();
  }
  ...
  if ( verbose() ) {
    report(EVT_INFO,"EvtGen") << "dGdsProbMax = " << probMax << " for s = "  << sProbMax << endl;
  }
  ...
  if ( verbose() ) {
   report(EVT_INFO,"EvtGen") << "dGdsdupProbMax = " << probMax << " for s = "  << sProbMax
			  << " and u = " << uProbMax << endl;
  }
...
void EvtBtoXsll::decay( EvtParticle *p ){
  ...
  while (xhadronMass < _mxmin)
  {
	...
	//	report(EVT_INFO,"EvtGen") << "nan from dGdsProb " << prob << " " << mb << " " << _ms << " " << ml << " " << xbox << std::endl;
      }
      if ( ybox < prob ) s=xbox;
      }
    ...
    while (tmp == 0.0)
    {
      ...
      if ( !(prob>=0.0) && !(prob<=0.0)) {
	report(EVT_INFO,"EvtGen") << "nan from dGdsProb " << prob << " " << mb << " " << _ms << " " << ml << " " << s << " " << u << std::endl;
      }
      if (prob > _dGdsdupProbMax && nmsg < 20)
      {
        report(EVT_INFO,"EvtGen") << "d2gdsdup GT d2gdsdup_max:" << prob
             << " " << _dGdsdupProbMax
