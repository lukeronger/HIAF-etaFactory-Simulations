#define PlutoEtaDarkPhotonEEG2evt_cxx
// The class definition in PlutoEtaDarkPhotonEEG2evt.h has been generated automatically
...
//
// root> T->Process("PlutoEtaDarkPhotonEEG2evt.C")
// root> T->Process("PlutoEtaDarkPhotonEEG2evt.C","some options")
// root> T->Process("PlutoEtaDarkPhotonEEG2evt.C+")
//
...
#include "PlutoEtaDarkPhotonEEG2evt.h"
#include <TH2.h>
#include <TStyle.h>
#include "TString.h"

void PlutoEtaDarkPhotonEEG2evt::Begin(TTree * /*tree*/)
{
   ...
   TString filename = Form("pp_to_ppeta_EEG_mDarkPhoton%lg_1.8GeV_Random2023.root.evt", mDarkPhoton);
   filename = "/data/rwang/simulation_works/HIAF-etaFactory-Simulations-localData2/Pluto6-evt/" + filename;
   evtout.open(filename.Data());
}


void PlutoEtaDarkPhotonEEG2evt::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t PlutoEtaDarkPhotonEEG2evt::Process(Long64_t entry)
{
   ...
   double masses2[2] = {0.000511, 0.000511};
   phase_generator->SetDecay(  (*phase_generator->GetDecay(1)), 2, masses2);
   weight = phase_generator->Generate();
   evtout<<"  "<<1<<"\t"<<-11<<"\t"<<1<<"\t"<<0<<"\t"<<0<<"\t"<<-1<<"\t"<<-1<<"\t";
   evtout<<phase_generator->GetDecay(0)->Px()<<" "<<phase_generator->GetDecay(0)->Py()<<" "<<phase_generator->GetDecay(0)->Pz()<<" "<<phase_generator->GetDecay(0)->E()<<" ";
   evtout<<0<<" "<<0<<" "<<0<<" "<<0<<endl;
   evtout<<"  "<<2<<"\t"<<11<<"\t"<<1<<"\t"<<0<<"\t"<<0<<"\t"<<-1<<"\t"<<-1<<"\t";
...
void PlutoEtaDarkPhotonEEG2evt::SlaveTerminate()
{
...
void PlutoEtaDarkPhotonEEG2evt::Terminate()
{
