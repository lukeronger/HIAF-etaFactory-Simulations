...
//      $Id: FsmDrcDisc.cc,v 1.7 2007/05/24 08:07:40 klausg Exp $
//
// Description:
//      Class FsmDrcBarrel
//      
//  Implementation of the Disc DIRC for the FastSim
//
...
ChnsFsmDrcDisc::ChnsFsmDrcDisc(ArgList &par) 
{
  initParameters();
...
ChnsFsmResponse* 
ChnsFsmDrcDisc::respond(ChnsFsmTrack *t)
{
  ...
  if (wasDetected && fabs(t->charge())>1e-8)
  {
    ...
    //double thtdeg=theta/M_PI*180.;  //move at ion forward side, need adapt.
    double thtdeg=theta/M_PI*180.;
    if(thtdeg>90)  thtdeg = 180-thtdeg;
    ...
    //double l = _dDisc/cos(theta);//  _dSlab*sqrt( 1/(sin(theta)*sin(theta)) + 1/(tan(psi)*tan(psi)) );   //l is negative at ion forward side... so fix it
    double l = _dDisc/fabs(cos(theta));
    
    // deteremine trapping fraction
    double trapped = _trap;
   
    if (npid>=0 && trapfrac[npid]) 
      trapped = npid<0 ? 0.0 : trapfrac[npid]->GetBinContent(trapfrac[npid]->FindBin(p<6.0?p:6.0,thtdeg<25?thtdeg:25));
    if (npid == 3&& trapfrac[npid]) //kaon histgram has problem!! need special treatment.
      trapped = npid<0 ? 0.0 : trapfrac[npid]->GetBinContent(trapfrac[npid]->FindBin(p<2.5?p:2.5,thtdeg<25?thtdeg:25)); 
    ...
    result->setDrcDiscThtc(measThetaC,sig);
...
double
ChnsFsmDrcDisc::gauss(double x, double x0, double s)
{
...
double
ChnsFsmDrcDisc::compThetaC(double p, double m)
{
  double val=0.0;
  if (p==0) return 0.0;
...
bool 
ChnsFsmDrcDisc::detected(ChnsFsmTrack *t) const
{
  if (t->hitMapValid()) {
    ...
    bool correctPidType=(lundId==11 || lundId==13 || lundId==211 || lundId==321 || lundId==2212);

    return ( p>p_cerenkov_min  && theta>=_thtMin && theta<=_thtMax && correctPidType && _rand->Uniform()<=_efficiency);
  }
}

void
ChnsFsmDrcDisc::print(ostream &o)
{
...
ChnsFsmDrcDisc::initParameters()
{
  ...
  _trap            = 0.7;
  _efficiency	   = 1.0; 
  _parFileName     = "$VMCWORKDIR/fsim/trapfrac_disc.root"; 
}

bool
ChnsFsmDrcDisc::setParameter(std::string &name,std::string &value)
{
  // *****************
...
bool
ChnsFsmDrcDisc::setParameter(std::string &name, double value)
{
  // *****************
...
bool ChnsFsmDrcDisc::readParameters()
{
  ...
  if (f->IsZombie()) 
  {
    cout <<" -W-  (ChnsFsmDrcDisc::readParameters) - file "<<_parFileName.c_str()
