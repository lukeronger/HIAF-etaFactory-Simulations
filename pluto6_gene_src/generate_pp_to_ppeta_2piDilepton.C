...

{
    ...
    TString filename = Form("../hiaf_MCdata/pp_to_ppeta_PipPimDilepton_%lgGeV", BeamEnergy);

    //makeDistributionManager()->Disable("pp_eta_pp_align");
    //makeDistributionManager()->Disable("pp_eta_prod_angle");
    //cout<<inputstr<<endl;
    //cout<<filename<<endl;
    PReaction my_reaction(inputstr, "p", "p", "p p eta [pi+ pi- e+ e-]", filename, 1, 0, 0, 0);
