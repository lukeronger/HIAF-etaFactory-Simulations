  ...
  if (t->hitMapValid()) {
    ...
    double charge=t->charge();
		if(fabs(charge)>0.001){   //  a) for charged particles --Yutie
			...
			if(lund == 22) return ( theta>=_thtMin && theta<=_thtMax && E>_Emin && phi>=_phiMin && phi<=_phiMax && _rand->Rndm()<=_efficiency);
...
ChnsFsmEmcBarrel::dE(ChnsFsmTrack *t) const
{
  ...
  return (sqrt(_aPar*_aPar+_bPar*_bPar/E ) * E);
}
...
ChnsFsmEmcBarrel::initParameters()
{
  ...
  _aPar=0.001; //2.79285e-2;
  _bPar=0.03; ///5.56776e-2;
  _cPar=7.7596e-3;
  //_aPar  = 0.01;
  //_bPar = 0.01;
  //_cPar = 0.01;
