...
contains
  ...
  subroutine DoElementaryAnalysis (realparticles, finalFlag)
    ...
    real, save :: sigma_pion(-1:1),sigma_Eta,sigma_kaon(0:1,1:2),sigma_antikaon(-1:0,1:2),sigma_KKbar(1:19),sigma_pipi(1:4)
    ...
    if (finalFlag) then
       ...
       if (ncall==isu) then
          ...
          write(31,'(6A)')&
               ...
               & '  pi-:   pi0:   pi+:  eta:   K+:    K-:    K0(ann,nonann):  Kbar0(ann,nonann):',&
       ...
       write(31,5) ekin_lab, p_lab, srts, sigma_total, sigma_elastic, sigma_CEX, sigma_PROD_pbarp,&
            & sigma_ANN, sigma_Y,&
            & sigma_pion(-1:1), sigma_Eta, sum(sigma_kaon(1,:)), sum(sigma_antikaon(-1,:)),&
...
5      format(38(1x,f11.6),101(1x,e13.6),1x,f11.6)
