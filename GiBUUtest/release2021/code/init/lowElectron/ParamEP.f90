...
module ParamEP
  ...
  public :: CalcParamEP
  public :: ParamEP_Bosted
  public :: ParamEP_BostedNonRes
  public :: ParamEP_ALLM
  public :: ParamEP_ALLM97
  public :: ParamEP_R1990
  public :: ParamEP_R_BY
  public :: Flux_Bosted
...
contains
  ...
  subroutine CalcParamEP(W,Q2,eps, XS, XSerr)
    ...
    case (1)
       if (present(XSerr)) then
          call ParamEP_Brasse(W,Q2,eps, XS, XSerr)
       else
          call ParamEP_Brasse(W,Q2,eps, XS)
       end if
    case (2)
       call ParamEP_Bosted(W,Q2,eps, XS)
  ...
  !****************************************************************************
  !****s* ParamEP/ParamEP_Brasse
  ! NAME
  ! subroutine ParamEP_Brasse(W,Q2,eps, XS, XSerr)
  !
  ! PURPOSE
...
!    write(*,'(1P,4e14.4)') W,Bra1(iSet,iW,1),XS
  ...
  !****************************************************************************
  !****s* ParamEP/ParamEP_Bosted
  ! NAME
  ! subroutine ParamEP_Bosted(W,Q2,eps, XS)
  !
  ! PURPOSE
...
!1000  format(8f12.5)
  ...
  end subroutine ParamEP_Bosted

  !****************************************************************************
  !****s* ParamEP/ParamEP_BostedNonRes
  ! NAME
  ! subroutine ParamEP_BostedNonRes(W,Q2, sigmaL,sigmaT)
  !
  ! PURPOSE
  ! Calculate the non-resonant XS according the Bosted Parametrization:
  ! * M. E. Christy and P. E. Bosted,
  !   ``Empirical Fit to Precision Inclusive Electron-Proton Cross Sections
  !   in the Resonance Region,'' Phys.Rev. C81 (2010) 055213
  !
  ...
  subroutine ParamEP_BostedNonRes(W,Q2, sigmaL,sigmaT)
    ...
    real, intent(in) :: W, Q2
    real, intent(out) :: sigmaL, sigmaT

    ! transversal parameters:
    real, parameter :: Q20T = 0.05
    real, dimension(2), parameter :: aT = (/ 0.0675,0.2098 /)
    real, dimension(2), parameter :: bT = (/ 1.3501,1.5715 /)
    real, dimension(2), parameter :: cT = (/ 0.1205,0.0907 /)
    real, dimension(2), parameter :: dT = (/-0.0038,0.0104 /)
    real, dimension(2), parameter :: sigT0 = (/ 246.1,-89.4 /)

    ! longitudinal parameters:
    real,parameter :: Q20L = 0.125
    real,parameter :: aL = 0.0
    real,parameter :: bL = 4.0294
    real,parameter :: cL = 3.1285
    real,parameter :: dL = 0.3340
    real,parameter :: eL = 4.9623
    real,parameter :: m0 = 4.2802
    real,parameter :: sigL0 = 86.7

    real, parameter :: Wthr = mN+mPi

    real :: xprimeT,xprimeL,t,x
    integer :: i
    ...
    ! limits of the fit:
    if (Q2 > 8.) return
    if (W < Wthr .or. W > 3.1) return

    ! transversal:
    do i=1,2
       sigmaT = sigmaT + sigT0(i)/((Q2 + aT(i))**(bT(i) + cT(i)*Q2 + dT(i)*Q2**2))*(W - Wthr)**(i+0.5)
    end do

    xprimeT = 1./(1. + (W**2 - Wthr**2)/(Q2 + Q20T))
    sigmaT = sigmaT * xprimeT

    ! longitudinal:
    t = Log(Log((Q2+m0)/0.33**2))/Log(m0/0.33**2)
    x = Q2/(W**2+Q2-mN**2)     ! = Q2/(2.*mN*nu)
    xprimeL = 1./(1. + (W**2 - Wthr**2)/(Q2 + Q20L))

    sigmaL = sigL0 * (1. - xprimeL)**(aL*t + bL)/(1 - x) * Q2**cL/(Q2+Q20L)**(1.+cL) * xprimeL**(dL+eL*t)


  end subroutine ParamEP_BostedNonRes


  !****************************************************************************
  !****s* ParamEP/ParamEP_ALLM
  ! NAME
  ! subroutine ParamEP_ALLM(W,Q2, XS)
  !
  ! PURPOSE
...
!      W2 = M2+Q2*(1d0/x-1d0)    ! for x as input
  ...
  end subroutine ParamEP_ALLM


  !****************************************************************************
  !****s* ParamEP/ParamEP_ALLM97
  ! NAME
  ! subroutine ParamEP_ALLM97(W,Q2, XS)
  !
  ! PURPOSE
...
!    x = 1d0/(1d0+(W2-M2)/Q2)  ! for W as input
  ...
  end subroutine ParamEP_ALLM97

  !****************************************************************************
  !****f* ParamEP/ParamEP_R1990
  ! NAME
  ! real function ParamEP_R1990(W,Q2)
  !
  ! PURPOSE
  ...
  real function ParamEP_R1990(W,Q2)
    ...
    ParamEP_R1990 = b1*(1+12*QQ*0.125**2/((QQ+1)*(0.125**2+x**2)))/log(QQ/Lam**2) &
         & + b2/QQ &
         & + b3/(QQ**2+0.3**2)

  end function ParamEP_R1990

  !****************************************************************************
  !****f* ParamEP/ParamEP_R_BY
  ! NAME
  ! real function ParamEP_R_BY(W,Q2)
  !
  ! PURPOSE
  ! Calculate R=sigma_L/sigma_T according:
  ! * A.Bodek, U.K.Yang,
  ...
  real function ParamEP_R_BY(W,Q2)
    ...
    ParamEP_R_BY = 0.0635/Log(Q2/0.04)*(1. + 12.*Q2/(Q2+1.0) * 0.125**2/(0.125**2+x**2)) + 0.5747/Q2 - 0.3534/(Q2**2+0.09)

    if (Q2 < 0.35) then
       ParamEP_R_BY = ParamEP_R_BY * 3.207*Q2/(Q2**2+1.)
    end if


  end function ParamEP_R_BY
