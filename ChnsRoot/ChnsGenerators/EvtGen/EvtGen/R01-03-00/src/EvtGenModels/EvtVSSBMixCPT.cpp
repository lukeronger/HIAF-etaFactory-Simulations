...
void EvtVSSBMixCPT::init(){
  ...
  if (getNArg()<1) {
    report(EVT_ERROR,"EvtGen") << "EvtVSSBMix generator expected "
  ...
  if ( getNDaug()==4) {
    if ( getDaug(0)!=getDaug(2)||getDaug(1)!=getDaug(3)){
      report(EVT_ERROR,"EvtGen") << "EvtVSSBMixCPT generator allows "
  ...
  if(!(EvtPDL::chargeConj(getDaug(0)) == getDaug(1))) {
    report(EVT_ERROR,"EvtGen") << "EvtVSSBMixCPT generator expected daughters "
  ...
  if ( verbose() ) {
    report(EVT_INFO,"EvtGen") << "VSS_BMIXCPT will generate mixing and CPT/CP effects in mixing:"
			  << endl << endl
