  ...
  while(rew){
	...
	if ( Eh > 0 && Eh < mB ) {
	  ...
	  if ( sh > _masscut*_masscut
	    ...
	    if ( y > 1 ) report(EVT_WARNING,"EvtVubHybrid") <<"EvtVubHybrid decay probability > 1 found: " << y << endl;
 	    if ( y >= xran ) tryit = 0;
	  }
	}
      }
...
double EvtVubHybrid::getWeight(double mX, double q2, double El) {
  ...
  if ( (ibin_mX < 0) || (ibin_q2 < 0) || (ibin_El < 0) ) {
    report(EVT_ERROR,"EvtVubHybrid") << "Cannot determine hybrid weight "
                                 << "for this event " 
...
void EvtVubHybrid::readWeights(int startArg) {
  ...
  if (maxw == 0) {
    report(EVT_ERROR,"EvtVubHybrid") << "EvtVub generator expected at least one " 
