...
module AnaEvent
 ...
 ! use neutrinoAnalysis, only: kineticEnergyDetectionThreshold_lepton, &
...
!   end function event_getMultiplicities
  ...
  subroutine event_sigma(E,sigmaOut,newInit,runNumber,identifier)
    ...
    use particleProperties, only: hadron, validCharge, PartName
...
!          if(E(j)%numberParticles(numStableMesons+1,0).ne.0) cycle
    ...
    idLoop_single: do i=lbound(particleIDs,dim=1),ubound(particleIDs,dim=1)
       chargeLoop_single: do charge=-2,2
          if (validCharge(particleIDs(i),charge)) then
...
!!$       end do
    ...
    idLoop: do i=lbound(particleIDs,dim=1),ubound(particleIDs,dim=1)
       ...
       chargeLoop: do charge=-2,2
          ...
          if (makeInitHists) then
             ...
             name = PartName(particleIDs(i),charge,.false.)
             if (exclusive_hadron)  name = trim('excl'//name)
 ...
 ! first loop over all particles (mesons and baryons) in array particleIDs in
 ...
 ! then loop over individual charge states
       chargeLoop: do charge=-2,2
          ...
          name = PartName(particleIds(i),charge,.false.)
          name1 = name
          if (exclusive_hadron) name1 = trim('excl'//name)
          if (makeInitHists) then
...
!    character(100) :: fname
  ...
  ! INPUTS
  ! * type(tAnaEvent), dimension(:) :: E -- List of events
  ! * real :: maxQ2, binsizeQ2 -- maximum value and binsize for lepton Q2
  ! * real :: AngleUpperDetectionThresholdDegrees_lepton
  ! * real :: AngleUpperDetectionThresholdDegrees_lepton
  ! * character(*) :: string -- used as prefix for all output files
  ! * integer      :: runNumber -- number of the run
  !
  ...
  subroutine event_dSigma_dLeptonVariables(E,maxQ2,binsizeQ2,AngleUpperDetectionThresholdDegrees_lepton, &
    ...
    type(tAnaEvent) , dimension(:) :: E ! List of events
    type(particle) :: part
    real, intent(in) :: maxQ2,binsizeQ2
    character(*), intent(in) :: string ! used as prefix for all output files
    integer, intent(in) :: runNumber ! number of the run
    ...
    type(histogram2D),optional, intent(inout) :: hist_dpLdpT
    ...
    type(histogram) :: dsigma_dEnu, dsigma_dE,dsigma_dcos, dsigma_dQ2, &
                       & dsigma_dQ2p
    type(histogram2D) :: dSigma_dEdCost,dSigma_dpLdpT
    integer :: j, prod_id
    character(80) :: filename_Enu, filename_E,filename_cos,filename_Q2,  &
                     & filename_Q2p,filename_Ecos,filename_pLpT
    character(13) :: name
    ...
    real, intent(in):: kineticEnergyDetectionThreshold_lepton
    real, intent(in):: AngleUpperDetectionThresholdDegrees_lepton
    ...
    if (makeInitHists) then
       ...
       call createHist(dsigma_dQ2,   &
        ...
        call createHist(dsigma_dQ2p,   &
          & 'dSigma/dQ^2( outgoing  lepton ) for events with 0pi, 1p' &
          &  //trim(name), &
          &  0.0, Elept_max, delta_Elept)
       call CreateHist2D(dSigma_dEdCost, &
          & 'd2sigma vs E(lepton) and cost (lepton) for 0pi events ' &
          & //trim(name), &
          & (/0.,cost_min/),(/Elept_max,cost_max/),(/delta_Elept,delta_cost/))
        call CreateHist2D(dSigma_dpLdpT, &
          & 'd2sigma vs pL(lepton) and pT (lepton) for 0pi events ' &
          & //trim(name), &
       ...
       dsigma_dEdcost = hist_dEdcost
       dsigma_dpLdpT = hist_dpLdpT
    end if
    ...
    if (sameFileName) then
       ...
       filename_Ecos=trim(string)//'_d2Sigma_dEdcost_lepton_'//&
             & trim(name)//'.dat'
       filename_pLpT=trim(string)//'_d2Sigma_dpLdpT_lepton_'//&
             & trim(name)//'.dat'
    else
       ...
       filename_Q2=trim(string)//'_dSigma_dQ2_lepton_'//trim(name)//'.'// &
        ...
        filename_Q2p=trim(string)//'_dSigma_dQ2p_lepton_'//trim(name)//'.'// &
          &  trim(intTochar(runNumber))//'.dat'
       filename_Ecos=trim(string)//'_d2Sigma_dEdcost_lepton'// &
          &  trim(name)//'.'// trim(intTochar(runNumber))//'.dat'
       filename_pLpT=trim(string)//'_d2Sigma_dpLdpT_lepton'// &
          &  trim(name)//'.'// trim(intTochar(runNumber))//'.dat'
    end if
...
!
        if (specificEvent==1) then
          call AddHist2D(dSigma_dEdCost,&
               & (/lep_mom(0),costheta/),&
               & perweight)
!write(*,*) 'elept=',lep_mom(0),'cost=',costheta,'pw=',perweight

          plept = abs3(lep_mom)
          pL = plept * costheta
          pT = plept * sqrt(1. - costheta**2)
          call AddHist2D(dSigma_dpLdpT,&
               & (/pL,pT/),&
               & perweight)
!write(*,*) 'plept=',plept,'cost=',costheta,'pL=',pL,'pT=',pT,'pw=',perweight
        end if
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  Now all histograms have been filled. In the following statistical errors are added
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (present(hist_dEdcost)) then
       hist_dEdcost = dsigma_dEdcost

!  the following lines do not work for 2D histograms
!       if (.not.makeInitHists) then
!          ! Calculate the square of the changes compared to last run:
!          call makeError_hist(hist_dEdcost,dsigma_dEdcost)
!          ! Save the list
!          hist_dEdcost = dsigma_dEdcost
!          ! Set yVal(:,3) to the statistical error before doing the output :
!          call setError_hist(dsigma_dEdcost,runNumber)
!       else
!          call makeError_hist(b=dsigma_dEdcost)
!          ! Save the list
!          hist_dEdcost=dsigma_dEdcost
!       end if
     end if

    if (present(hist_dpLdpT)) then
       hist_dpLdpT = dsigma_dpLdpT

!  the following lines do not work for 2D histograms
!       if (.not.makeInitHists) then
!          ! Calculate the square of the changes compared to last run:
!          call makeError_hist(hist_dpLdpT,dsigma_dpLdpT)
!          ! Save the list
!          hist_dpLdpT = dsigma_dpLdpT
!          ! Set yVal(:,3) to the statistical error before doing the output :
!          call setError_hist(dsigma_dpLdpT,runNumber)
!       else
!          call makeError_hist(b=dsigma_dpLdpT)
!          ! Save the list
!          hist_dpLdpT=dsigma_dpLdpT
!       end if
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (specificEvent==1) then
       ...
       if (makeInitHists) then
          call WriteHist2D_Gnuplot(dsigma_dEdcost,12,0.,1.)
       else
          ! normalize to number of runs
          call WriteHist2D_Gnuplot(dsigma_dEdcost,12,0.,1./float(runNumber))
       end if
       close(12)
    end if

    if (specificEvent==1) then
       open(12,file=filename_pLpT)
       if (makeInitHists) then
          call WriteHist2D_Gnuplot(dsigma_dpLdPT,12,0.,1.)
       else
          ! normalize to number of runs
          call WriteHist2D_Gnuplot(dsigma_dpLdpT,12,0.,1./float(runNumber))
       end if
       close(12)
    end if
    ...
    call RemoveHist2D(dsigma_dEdcost)
    call RemoveHist2D(dsigma_dpLdpT)
       ...
       do
          ...
          if (NeutrinoProdID) then
             if (.not.neutrinoProdInfo_Get(i,prod_id,perweight,lepIn_mom,  &
                & lepton_mom,boson_mom)) then
                write(*,*) 'error in getting production info in event_dump, stop'
                stop
             end if
             enu=lepton_mom(0)+boson_mom(0)
             if (printParticle) write(47,out) run,i,factor*pNode%V%ID,pNode%V%Charge,  &
                  &  pNode%V%perweight, &
                  &  pNode%V%position,pNode%V%momentum &
                  & ,pNode%V%history,prod_id, enu
          else
             if (printParticle) write(47,outShort) run,i,factor*pNode%V%ID,pNode%V%Charge, &
