...
contains
  ...
  subroutine predictorStep(Parts,gradP,gradR,delta_T)
    use offshellPotential, only: &
         get_useOffShellPotentialBaryons,get_useOffShellPotentialMesons,SetOffShellEnergy
    use particleDefinition
    use minkowski, only:SP
    use output
    ...
    do iEns=lbound(Parts,dim=1),ubound(Parts,dim=1)
       do iPart=lbound(Parts,dim=2),ubound(Parts,dim=2)
          ...
          if(FreezeNonint .and. .not.pPart%perturbative .and.pPart%event(1)<1000000) then
             pPart%velocity=0.
             cycle
          end if
  ...
  subroutine propagate_cascade(Parts,delta_T)
    ...
    defreezeFlag = eventtype==InABox .or. eventtype==InABox_pion .or. eventtype==InABox_delta .or. eventtype==Box &
    ...
    do iEns=lbound(Parts,dim=1),ubound(Parts,dim=1)
       do iPart=lbound(Parts,dim=2),ubound(Parts,dim=2)
          ...
          if (.not.defreezeFlag .and. .not.pPart%perturbative .and. 0<pPart%event(1) .and. pPart%event(1)<1000000) then
             ...
             if (mod(pPart%event(1),2)==1) then
                pPart%velocity(1:3) = t_vel(1:3)
             else
                pPart%velocity(1:3) = p_vel(1:3)
             end if
