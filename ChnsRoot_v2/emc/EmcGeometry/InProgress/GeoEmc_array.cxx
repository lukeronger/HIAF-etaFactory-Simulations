...
Bool_t EiccEmc::ProcessHits(FairVolume* vol)
{
	...
	if (TVirtualMC::GetMC()->IsTrackExiting() || TVirtualMC::GetMC()->IsTrackStop()
		...
		Int_t fModuleID = 0;        Int_t fSegmentID = 0;   Int_t fDetectorID = 0;     
		copyNo= -1;
		...
		AddHit( 
				...
				fELoss,
				fEventID,
				fModuleID, fSegmentID, fDetectorID, copyNo);
...
void EiccEmc::ConstructGeometry()
{
	...
	//endcap
	const int N_layer=240, N_row=7, N_col=7; // satisfy +-35 degree
...
EiccEmcPoint* EiccEmc::AddHit(
		...
		Double_t eLoss,
		Int_t eventID,
		Int_t moduleID,  Int_t segmentID, Int_t fDetectorID, Int_t copyID)
{
	TClonesArray& clref = *fEiccEmcPointCollection;
	Int_t size = clref.GetEntriesFast();
	return new (clref[size]) EiccEmcPoint(trackID, detID, pos, mom, time, length, eLoss, eventID, moduleID, segmentID, fDetectorID, copyID);
}
