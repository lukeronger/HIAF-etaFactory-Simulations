...
//	CHNS->AddNode(cylinder,1);
	...
	for(int i=0;i<5;i++){
		...
		cylinder->SetLineColor(kBlue-4); //kCyan);
        ...
        for(int i=0;i<5;i++){
                ...
                cylinder1->SetLineColor(kBlue-4); //kCyan);
...
//	Tof1->SetTransparency(99);
	...
	EMC1->SetLineColor(kGreen-9);
	EMC1->SetTransparency(0);
	...
	EMC2->SetLineColor(kGreen-9);
	EMC2->SetTransparency(0);
