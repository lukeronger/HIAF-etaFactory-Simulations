...
!       call plot_f(part)
  ...
  contains
    ...
    real function m_eff_sqr(partIn, density, enerOut)
      ...
      ! (1.2) Evaluate scalar potential V_S:
      ...
      if (treatParticleOffShell(part%ID,part%OffShellParameter)) then
         ...
         ! (1.3) Boost from calculation frame to LRF
         call boostToLRF(part,1,density)
         part%momentum(0)=FreeEnergy(part)+potential_LRF(part,density,addCoulomb=.false.)
      end if

      if (DoCoulomb) then
         ! (2.1) Boost from LRF to calculation frame
         call boostToLRF(part,2,density)
         ! (2.2) add Coulomb potential
         part%momentum(0)=part%momentum(0) &
              + emfoca(part%position,part%momentum(1:3),part%charge,part%ID)
      end if
