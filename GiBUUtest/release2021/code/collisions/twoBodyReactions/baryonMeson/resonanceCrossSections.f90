...
!    use densityModule, only: densityAt
    ...
    resonanceMass = sqrt( mom(0)**2 - sum(mom(1:3)**2) )
...
!    call traceback()
  ...
  ! NAME
  ! function barMes2resonance(idMeson_ini, idBaryon_ini, chargeMeson_ini,
  ! chargeBaryon_ini, propagated, mediumAtCollision, momentumLRF, masses,
  ! mesonMass, baryonMass, position, perturbative, srts) result (sigma)
  ! PURPOSE
  ! Evaluates baryon meson -> Resonance cross sections by summing over the
  ! resonances final state.
  ! One gets baryon meson -> X by summing over the (baryon meson -> R-> X)
  ! channels for all resonances.
  !
  ...
  ! * logical :: propagated
  !   --- if .true., then onsider only propagated particles in final state,
  !   otherwise consider all particles in final state
  ...
  !   --- Cross section for each intermediate baryon resonance in units of mB.
  !   The index denotes the resonance.
  ! * real, dimension(Delta:nbar) :: masses
  !   --- Masses of the produced resonances; index denotes the resonance.
  !****************************************************************************
  function barMes2resonance(idMeson_ini, idBaryon_ini, chargeMeson_ini, &
       chargeBaryon_ini, propagated, mediumAtColl, &
       momLRF, masses, mesonMass, baryonMass, position, perturbative, srts) &
       result (sigma)
    ...
    integer, intent(in)              :: idMeson_ini, idBaryon_ini, &
         chargeMeson_ini, chargeBaryon_ini
    ...
    do resId=Delta,nbar
       ...
       gamma_In = partialWidthBaryonMedium(resID,masses(resID),.true.,&
            IDmeson_ini, IDbaryon_ini,momLRF,vacuum,baryonMass,mesonMass)
       gammaTot = WidthBaryonMedium (resID,masses(resID),momLRF,mediumAtColl)

       if (abs(gamma_In)<1E-6 .or. abs(gammaTot)<1E-6) cycle

       if (resID==Delta.and.debug) write(300,'(6F8.4)') &
            momCM2, gamma_In, gammaTot, masses(resID)
  ...
  ! NAME
  ! real function barMes_R_barMes(idMes_in, idBar_in, idMes_out, idBar_out,
  ! chargeMes_in, chargeBar_in, chargeMes_out, chargeBar_out,
  ! background, propagated, MediumAtCollision, momentumLRF,
  ! mesonMass_in, baryonMass_in, position, perturbative, srts)
  !
  ! PURPOSE
  ...
  ! * logical :: background ---
  !   .true. = Compute background cross section : Therefore consider only
  !   non-propagated particles in intermediate state;
  ...
  ! * type(medium) :: mediumAtCollision --- Medium information at the point
  !   of collision
  ! * real, dimension(0:3) :: momentumLRF --- Total  Momentum in LRF =
  !   Momentum of resonance in LRF
  ...
  !
  ! NOTES
  ! Note that it is not useful to set both background and propagated
  ! to .true. : Result=0.
  ! RESULT
  ! * resonance contribution to baryon meson -> baryon meson (in mb)
  !****************************************************************************
  function barMes_R_barMes(idMes_in, idBar_in, idMes_out, idBar_out, &
       chargeMes_in, chargeBar_in, chargeMes_out, chargeBar_out, &
       background, propagated, mediumAtColl, momLRF, &
       mesonMass_in, baryonMass_in, &
    ...
    type(medium), intent(in)   :: mediumAtColl
    real, intent(in)           :: momLRF(0:3), mesonMass_in, baryonMass_in, &
         position(1:3)
    logical, intent(in)        :: perturbative
    real, intent(in), optional :: srts
    real :: sigma
    ...
    do resID = Delta, nbar
       ...
       if (.not. getRMF_flag()) then
          massRes = resonanceMass(resID, chargeRes, mediumAtColl, momLRF, &
       ...
       momCM2 = pCM_sqr(massRes**2, mesonMass_in**2, baryonMass_in**2)
       if (momCM2 <= 0.) cycle
       ...
       ! Use inWidth since there are both masses of the incoming particles
       ! given.
       ! And don't use medium since only the out-width shall be dressed due
       ! to the final states.

       Gamma_In = partialWidthBaryonMedium(resID, massRes, .true., &
            idMes_in, idBar_in, momLRF, vacuum, baryonMass_in, mesonMass_in)

       Gamma_Out = partialwidthBaryonMedium(resID, massRes, .false., &
            idMes_out, idBar_out, momLRF, mediumAtColl)
  ...
  ! *  2 : pi+ pi+ in final state
  !****************************************************************************
  subroutine sigma_npi_n2pi_resonances(srts, charge_iniPion, background, &
       sigmaTotal)
      ...
      real :: gammaTot, momCM, gamma_In, spinFactor, absMom_LRF, fullMass, &
           PI_REAL, PI_IMAG, energy
      integer :: resID,decID
      type(medium) :: med
