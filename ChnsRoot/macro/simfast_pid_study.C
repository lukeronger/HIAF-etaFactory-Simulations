...
int simfast_pid_study(int nevts=0, int seed_num = 1000)
{
  TStopwatch timer;
  timer.Start();
  gDebug=0;
  ...
  TString outfile;
  if (outfile=="") {
	outfile="singletracks_piKp_TPC_"; //1.root";
  	outfile+= seed_num;
	outfile+=".root";
  }
...
*/
  if(UseEvtGen){
    FairEvtGenGenerator* evtGen = new FairEvtGenGenerator("psi2s_jpsi2pi_1k.evt");
