...
public:
  ...
  virtual void init()
  {
    ...
    if(VERBOSE) report(EVT_INFO,"EvtGen") << "Create factory and interval" << std::endl;
    ...
    _nScan = parser.nScan();
    if(VERBOSE) report(EVT_INFO,"EvtGen") << "Pdf maximum " << _probMax << std::endl;
    if(VERBOSE) report(EVT_INFO,"EvtGen") << "Scan number " << _nScan << std::endl;    
  }
  ...
  virtual void decay(EvtParticle *p)
  {
    ...
    if(v.size() != N) {
      ...
      report(EVT_INFO,"EvtGen") << "Number of daughters " << N << std::endl;
      report(EVT_INFO,"EvtGen") << "Momentum vector size " << v.size() << std::endl;
      assert(0);
    }
