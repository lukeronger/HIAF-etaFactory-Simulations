...
!  if (debug) write(*,*) 'Total 4-momentum before collision:', momentum_calc
  ...
  if ( .not.accept_event((/part1,part2/),finalState) ) then
     collFlag = .false.
     return
  end if
...
!            &sum(finalState(:)%momentum(2)),sum(finalState(:)%momentum(3))
  ...
  do i = 1,maxId+1
     ...
     finalstate(i)%offshellparameter = getOffShellParameter( &
               & finalstate(i)%ID, finalstate(i)%Mass, &
               & finalstate(i)%momentum, finalstate(i)%position, successFlag)
     if (.not.successFlag) then
         collFlag = .false.
         return
     end if
     if (.not.getRMF_flag() .or. abs(finalstate(i)%offshellparameter).gt.1.E-06) then
