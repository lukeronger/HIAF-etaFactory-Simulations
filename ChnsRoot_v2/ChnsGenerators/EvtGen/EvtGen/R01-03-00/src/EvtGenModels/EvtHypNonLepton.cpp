...
void EvtHypNonLepton::init(){
  ...
  if(getNArg()<2 || getNArg()>3){ // alpha phi gamma delta
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton generator expected 2 or 3 arguments but found: " << getNArg() << std::endl;
    report(EVT_INFO,"EvtGen") << "  1. Decay asymmetry parameter - alpha" << std::endl;
    report(EVT_INFO,"EvtGen") << "  2. Parameter phi - in degrees (not radians)" << std::endl;
    report(EVT_INFO,"EvtGen") << "  3. Note on every x-th decay" << std::endl;
    ::abort();
  }

  if(getNDaug()!=2){ // Check that there are 2 daughters only
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton generator expected 2 daughters but found: " << getNDaug() << std::endl;
    ::abort();
  }

  // Check particles spins
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getParentId()))!=1){
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton generator expected dirac parent particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getParentId())) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(0)))!=1){
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton generator expected the first child to be dirac particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(0))) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  if(EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(1)))!=0){
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton generator expected the second child to be scalar particle, but found " << EvtSpinType::getSpin2(EvtPDL::getSpinType(getDaug(1))) << " spin degrees of freedom" << std::endl;
    ::abort();
  }
  ...
  if(m1+m2>=M){
    report(EVT_ERROR,"EvtGen") << " ERROR: EvtHypNonLepton found impossible decay: " << M << " --> " << m1 << " + " << m2 << " GeV\n" << std::endl;
    ::abort();
  }
...
void EvtHypNonLepton::initProbMax(){
  ...
  setProbMax(maxProb);
  report(EVT_INFO,"EvtGen") << " EvtHypNonLepton set up maximum probability to " << maxProb << std::endl;
...
void EvtHypNonLepton::calcAmp(EvtAmp *amp,EvtParticle *parent){
  ...
  if(m_noTries>0) if(!((++noTries)%m_noTries)) report(EVT_DEBUG,"EvtGen") << " EvtHypNonLepton already finished " << noTries << " matrix element calculations" << std::endl;
}
