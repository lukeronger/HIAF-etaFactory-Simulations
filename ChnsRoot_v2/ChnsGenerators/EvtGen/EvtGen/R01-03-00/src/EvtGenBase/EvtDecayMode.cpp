...
EvtDecayMode::EvtDecayMode(const char* decay)
{
  ...
  if(i == string::npos) {
    ...
    report(EVT_INFO,"EvtGen") << "No non-space character found" << endl;
    assert(0);
  }

  if(j == string::npos) {
    
    report(EVT_INFO,"EvtGen") << "No space before -> found" << endl;
    assert(0);
  }
  ...
  if(i != j) {
    ...
    report(EVT_INFO,"EvtGen") << "Multiple mothers?" << i << "," << j << endl;
    assert(0);
  }
  j += 2;
