...
void EvtVectorIsr::decay( EvtParticle *p ){
  ...
  if (!firstorder){
	...
	if (n > 10000){
	  report(EVT_INFO,"EvtGen") << "EvtVectorIsr is having problems. Called ckhrad 10000 times.\n";
	  assert(0);
	}
      }
      ...
      if (phi->mass() > wcm_new){
	report(EVT_INFO,"EvtGen") << "EvtVectorIsr finds Vector mass="<<phi->mass()<<" > Weff=" << wcm_new<<".  Should not happen\n";
	assert(0);
      }
	...
	cs_Born = W/s_new;
      ...
      if (f > fmax && fmax > 0.){
	  report(EVT_INFO,"EvtGen") << "EvtVectorIsr finds a problem with fmax, the maximum weight setting\n"
	...
	if (f>largest_f) {
	  largest_f = f;
	  report(EVT_INFO,"EvtGen")  << m << " " <<  EvtPDL::name(phi->getId()) << " "
	       << "vector_mass " 
	...
	if (fmax > 0.) report(EVT_INFO,"EvtGen") << "EvtVectorIsr is having problems. Check the fmax value - the 3rd argument in the .dec file\n"
