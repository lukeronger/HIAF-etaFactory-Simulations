...
module output
  ...
  public :: WriteParticleCollisionList
...
!    character*(*), parameter :: FormatMom = '4e11.3'
    ...
    character(250), parameter :: FormatPart2 = &
       ...
       & //'1P,e11.3," time:",0P,3f9.3,l2," parents:",3i4,1P,3e11.3)'
...
!!$         Part%perWeight
    ...
    if (present(iEnsemble) .and. present(iPart) .and. present(Part)) then
       ...
       if (useProductionPos) then
          write(iFile,'(3A)') 'Ensemb| Index[   Number]: ID IQ Anti mass | momentum(0:3) ',&
               '| position(1:3) | ScaleCS firstEvent [event] perWeight, max(-9.9d0,formationTime), ',&
               'productionTime,lastCollisionTime, in_Formation, parents, productionPos'
       else
          write(iFile,'(3A)') 'Ensemb| Index[   Number]: ID IQ Anti mass | momentum(0:3) ',&
               '| position(1:3) | ScaleCS firstEvent [event] perWeight, max(-9.9d0,formationTime), ',&
               'productionTime,lastCollisionTime, in_Formation, parents'
       end if
    end if
  ...
  subroutine WriteParticle_debug(Part, med, densBar)
    ...
    if (present(densBar)) write(*,'(A,1P,4E17.9)') ' DensBar=',densBar
    write(*,'(79("-"))')
    write(*,*)
  ...
  !****************************************************************************
  !****s* output/WriteParticleCollisionList
  ! NAME
  ...
  subroutine WriteParticleCollisionList(iFile, iEns, Part)
    ...
    character*(*), parameter :: FormatMom = '4e15.7'
    character(250), parameter :: FormatPart = &
         & '(i5," ",i9,i4,i3,l3," ",f7.3,1P," ",'//FormatMom// &
         & ',3e11.3,e11.3,0P)'
  ...
  end subroutine WriteParticleCollisionList
