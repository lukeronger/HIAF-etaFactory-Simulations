...
void EvtEvalHelAmp::evalAmp( EvtParticle *p, EvtAmp& amp){
  ...
  if (fabs(prob1-prob2)>0.000001*prob1){
    report(EVT_INFO,"EvtGen") << "prob1,prob2:"<<prob1<<" "<<prob2<<endl;
    ::abort();
  }
...
void EvtEvalHelAmp::setUpRotationMatrices(EvtParticle* p,double theta, double phi){
  ...
  default:
    report(EVT_ERROR,"EvtGen") << "Spin2(_JA2)="<<_JA2<<" not supported!"<<endl;
    ::abort();
  }
  ...
  default:
    report(EVT_ERROR,"EvtGen") << "Spin2(_JB2)="<<_JB2<<" not supported!"<<endl;
    ::abort();
  }
  ...
  default:
    report(EVT_ERROR,"EvtGen") << "Spin2(_JC2)="<<_JC2<<" not supported!"<<endl;
    ::abort();
  }
