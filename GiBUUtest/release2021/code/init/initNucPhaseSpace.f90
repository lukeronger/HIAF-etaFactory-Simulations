...
module initNucleus_in_PS
  ...
  !****************************************************************************
  !****g* initNucleus_in_PS/zeroNucleusMomentum
  ! SOURCE
  !
  logical, save :: zeroNucleusMomentum = .true.
  !
  ! PURPOSE
  ! Indicate whether a procedure should be called to try to find a momentum
  ! init where the sum of all nucleon momenta (per ensemble) is zero (or at
  ! least close to zero).
  !
  ! At the moment, only a hill climbing algorithm is available, which changes
...
contains
  ...
  subroutine init
    ...
    if (zeroNucleusMomentum) then
       write(*,*) 'Nucleus momentum: reduced/close to zero'
    else
    ...
    if (nuc%Mass==1) then
       ...
       if (fullensemble) then
          ...
          if (producedProtons/=nuc%charge) then
             write(*,*) 'Problem in initPhaseSpace', &
                  producedProtons, nuc%charge
          end if
       else
          !********************************************************************
          ! Parallel ensembles
          ...
          do iEns=1,size(teilchen,dim=1)  !Loop over all ensembles
             ...
             if (improvedMC) then
                if (mod(iEns,100)==0) &
                     write(*,*) 'Ensemble',iEns,' of ',size(teilchen,dim=1)
             end if
             if (producedProtons/=nuc%charge) then
                write(*,*) 'Problem in initPhaseSpace', &
                     producedProtons, nuc%charge
       ...
       ! guarantee that no holes are in particle vector:
       ...
       ! choose momentum for a nucleus at rest:
       ...
       if (zeroNucleusMomentum) call reduceNucleusMomentum

       ! now boost the particles according nucleus velocity:
  ...
  contains
    ...
    !**************************************************************************
    !****s* initNucPhaseSpace/reduceNucleusMomentum
    ! NAME
    ! subroutine reduceNucleusMomentum
    ! PURPOSE
    ! This routine loops over all all nucleons and replaces rnadomly the
    ! direction of the nucleon momentum, if the nucleus momentum (i.e. the
    ! sum over all nucleon momenta in this ensemble) gets smaller by this.
    ! The number of loops is chosen as 'const/A'.
    !
    ...
    subroutine reduceNucleusMomentum
      ...
      integer, parameter :: nLoopMax = 2000 ! for resulting momenta ~ 10 MeV

      nLoop = nLoopMax/nuc%mass

      write(*,*) 'Running reduceNucleusMomentum with nLoop=',nLoop
