#define GiBUU_etaDecay2evt_cxx
// The class definition in GiBUU_etaDecay2evt.h has been generated automatically
...
//
// root> T->Process("GiBUU_etaDecay2evt.C")
// root> T->Process("GiBUU_etaDecay2evt.C","some options")
// root> T->Process("GiBUU_etaDecay2evt.C+")
//


#include "GiBUU_etaDecay2evt.h"
#include <TH2.h>
#include <TStyle.h>
...
void GiBUU_etaDecay2evt::Begin(TTree * /*tree*/)
{
   ...
   file_evt = new ofstream(directory_path + "GiBUU_final_states_etaDecayEEG.evt");
...
void GiBUU_etaDecay2evt::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t GiBUU_etaDecay2evt::Process(Long64_t entry)
{
   ...
   if(pips.size()>=1  &&  pims.size()>=1  &&  pi0s.size()>=1){
	   for(int i=0; i<pips.size(); i++)
		   for(int j=0; j<pims.size(); j++)
			   for(int k=0; k<pi0s.size(); k++){
				   vector4 = pips.at(i) + pims.at(j) + pi0s.at(k);   
				   if(fabs(vector4.M()-0.547)<0.01 ){
					   etas.push_back(vector4);
   ...
   if(Ntot>0){
	   ...
	   //for(int j=0; j<1; j++){
			TLorentzVector W(etas[j].Px(), etas[j].Py(), etas[j].Pz(), etas[j].E());
			double dielectronmass = dileptonMassDist_TF1->GetRandom(2*0.000510999+0.0001, etas[j].M()-0.0001);
...
void GiBUU_etaDecay2evt::SlaveTerminate()
{
...
void GiBUU_etaDecay2evt::Terminate()
{
