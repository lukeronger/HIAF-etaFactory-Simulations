...
void EvtCPUtil::fractB0nonCP(EvtComplex Af, EvtComplex Abarf, 
  ...
  //double gamma_B = EvtPDL::getWidth(B0);
  //report(EVT_INFO,"EvtGen") << "gamma " << gamma_B<< endl;
  //double xd = deltam/gamma_B;
...
void EvtCPUtil::OtherCoherentB( EvtParticle *p,double &t, EvtId &otherb, double probB0){
  ...
  if (parent==0||parent->getId()!=UPS4S) {
    ...
    if (parent!=0) {
      report(EVT_INFO,"EvtGen") << "p="<<EvtPDL::name(p->getId())
  ...
  if (parent != 0 ) {
    ...
    //if (entryCount>1){
    //  report(EVT_INFO,"EvtGen") << "Double CP decay:"<<entryCount<<endl;
    //}
	...
	scalar_part->init(B0B,p_init);
      ...
      if (decayed){
	//report(EVT_INFO,"EvtGen") << "In CP Util calling decay \n";
	other->decay();
      }
    ...
    report(EVT_INFO,"EvtGen") << "We have an error here!!!!"<<endl;
    otherb = EvtId(-1,-1); 
  }
